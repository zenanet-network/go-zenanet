...
balanceInit=$(docker exec eirene0 bash -c "eirene attach /var/lib/eirene/data/eirene.ipc -exec 'Math.round(web3.fromWei(eth.getBalance(eth.accounts[0])))'")
...
do
    ...
    balance=$(docker exec eirene0 bash -c "eirene attach /var/lib/eirene/data/eirene.ipc -exec 'Math.round(web3.fromWei(eth.getBalance(eth.accounts[0])))'")

    if ! [[ "$balance" =~ ^[0-9]+$ ]]; then
        echo "Something is wrong! Can't find the balance of first account in eirene network."
        exit 1
    fi
