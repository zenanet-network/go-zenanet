...
const (
	...
	MaxCodeSizePostAhmedabad = 32768           // Maximum bytecode to permit for a contract post Ahmedabad hard fork (eirene / polygon pos) (32KB)
	...
	// EireneDefaultMinerGasPrice defines the minimum gas price for eirene validators to mine a transaction.
	EireneDefaultMinerGasPrice = 25 * GWei

	// EireneDefaultTxPoolPriceLimit defines the minimum gas price limit for eirene to enforce txs acceptance into the pool.
	EireneDefaultTxPoolPriceLimit = 25 * GWei

	// EireneDefaultGpoIgnorePrice defines the minimum gas price below which eirene gpo will ignore transactions.
...
func BaseFeeChangeDenominator(eireneConfig *EireneConfig, number *big.Int) uint64 {
	if eireneConfig.IsDelhi(number) {
