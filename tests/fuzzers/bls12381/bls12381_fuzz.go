...
func fuzzCrossPairing(data []byte) int {
	...
	if !(bytes.Equal(res, cResult.Marshal())) {
		panic("pairing mismatch blst / gze")
	}
...
func fuzzCrossG1Add(data []byte) int {
	...
	if !(bytes.Equal(cp.Marshal(), bl3.Serialize())) {
		panic("G1 point addition mismatch blst / gze ")
	}
...
func fuzzCrossG1MultiExp(data []byte) int {
	...
	for i := 0; i < 17; i++ {
		// note that gze/crypto/bls12381 works only with scalars <= 32bytes
...
func fuzzCrossG1Mul(data []byte) int {
	...
	resGze := (new(gnark.G1Affine)).ScalarMultiplication(gp, scalar)

	if !bytes.Equal(resGze.Marshal(), resBl.Serialize()) {
		panic("bytes(blst.G1) != bytes(gze.G1)")
	}
	return 1
}
...
func fuzzCrossG2Mul(data []byte) int {
	...
	resBl := blp.Mult(blScalar)
	resGze := (new(gnark.G2Affine)).ScalarMultiplication(gp, scalar)
...
func getG1Points(input io.Reader) (*gnark.G1Affine, *blst.P1Affine, error) {
	...
	if !bytes.Equal(blstRes, cpBytes) {
		panic(fmt.Sprintf("bytes(blst.G1) != bytes(gze.G1)\nblst.G1: %x\ngze.G1: %x\n", blstRes, cpBytes))
	}
