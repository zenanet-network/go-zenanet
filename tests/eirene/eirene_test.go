...
import (
	...
	"github.com/zenanet-network/go-zenanet/common/fdlimit"
...
func TestForkWithBlockTime(t *testing.T) {
	...
	for _, test := range cases {
		t.Run(test.name, func(t *testing.T) {
			t.Parallel()
			genesis := InitGenesis(t, faucets, "./testdata/genesis_2val.json", 8)
			genesis.Config.Eirene.Sprint = test.sprint
			genesis.Config.Eirene.Period = test.blockTime
			genesis.Config.Eirene.BackupMultiplier = test.blockTime
			genesis.Config.Eirene.ProducerDelay = test.producerDelay
...
func TestInsertingSpanSizeBlocks(t *testing.T) {
	...
	_eirene := engine.(*eirene.Eirene)

	_, currentSpan := loadSpanFromFile(t)

	h, ctrl := getMockedHarmoniaClient(t, currentSpan)
	defer func() {
		_eirene.Close()
		ctrl.Finish()
	}()
	...
	_eirene.SetHarmoniaClient(h)

	block := init.genesis.ToBlock()
	// to := int64(block.Header().Time)

	currentValidators := []*validset.Validator{validset.NewValidator(addr, 10)}

	spanner := getMockedSpanner(t, currentValidators)
	_eirene.SetSpanner(spanner)

	// Insert sprintSize # of blocks so that span is fetched at the start of a new sprint
	for i := uint64(1); i <= spanSize; i++ {
		block = buildNextBlock(t, _eirene, chain, block, nil, init.genesis.Config.Eirene, nil, currentValidators)
	...
	validators, err := _eirene.GetCurrentValidators(context.Background(), block.Hash(), spanSize) // check validator set at the first block of new span
	if err != nil {
		t.Fatalf("%s", err)
	}
...
func TestFetchStateSyncEvents(t *testing.T) {
	...
	// B. Before inserting 1st block of the next sprint, mock harmonia deps
	...
	block = buildNextBlock(t, _eirene, chain, block, nil, init.genesis.Config.Eirene, nil, res.Result.ValidatorSet.Validators)
...
func TestFetchStateSyncEvents_2(t *testing.T) {
	...
	res.Result.ValidatorSet.Validators = append(res.Result.ValidatorSet.Validators, validset.NewValidator(addr, 4500))
	...
	var currentValidators []*validset.Validator
	...
	lastStateID, _ := _eirene.GenesisContractsClient.LastStateId(nil, sprintSize, block.Hash())
	...
	lastStateID, _ = _eirene.GenesisContractsClient.LastStateId(nil, spanSize, block.Hash())
...
func TestOutOfTurnSigning(t *testing.T) {
	...
	_, harmoniaSpan := loadSpanFromFile(t)
	proposer := validset.NewValidator(addr, 10)
	harmoniaSpan.ValidatorSet.Validators = append(harmoniaSpan.ValidatorSet.Validators, proposer)

	// add the block producer
	h, ctrl := getMockedHarmoniaClient(t, harmoniaSpan)
	defer ctrl.Finish()
	...
	spanner := getMockedSpanner(t, harmoniaSpan.ValidatorSet.Validators)
	_eirene.SetSpanner(spanner)
	...
	setDifficulty := func(header *types.Header) {
		if IsSprintStart(header.Number.Uint64()) {
			header.Difficulty = big.NewInt(int64(len(harmoniaSpan.ValidatorSet.Validators)))
		}
	}

	for i := uint64(1); i < spanSize; i++ {
		block = buildNextBlock(t, _eirene, chain, block, nil, init.genesis.Config.Eirene, nil, harmoniaSpan.ValidatorSet.Validators, setDifficulty)
		insertNewBlock(t, chain, block)
	}
	...
	setDifficulty = func(header *types.Header) {
		header.Difficulty = big.NewInt(int64(len(harmoniaSpan.ValidatorSet.Validators)) - turn)
	}

	block = buildNextBlock(t, _eirene, chain, block, signerKey, init.genesis.Config.Eirene, nil, harmoniaSpan.ValidatorSet.Validators, setParentTime, setDifficulty)
	_, err := chain.InsertChain([]*types.Block{block})
	require.Equal(t,
		eirene.BlockTooSoonError{Number: spanSize, Succession: expectedSuccessionNumber},
		*err.(*eirene.BlockTooSoonError))

	expectedDifficulty := uint64(len(harmoniaSpan.ValidatorSet.Validators) - expectedSuccessionNumber - turn) // len(validators) - succession
	header := block.Header()

	diff := eirene.CalcProducerDelay(header.Number.Uint64(), expectedSuccessionNumber, init.genesis.Config.Eirene)
	header.Time += diff

	sign(t, header, signerKey, init.genesis.Config.Eirene)
	...
	require.Equal(t,
		eirene.WrongDifficultyError{Number: spanSize, Expected: expectedDifficulty, Actual: 3, Signer: newAddr.Bytes()},
		*err.(*eirene.WrongDifficultyError))
...
func TestSignerNotFound(t *testing.T) {
	...
	_eirene.Authorize(newAddr, func(account accounts.Account, s string, data []byte) ([]byte, error) {
		return crypto.Sign(crypto.Keccak256(data), newKey)
	})

	block = buildNextBlock(t, _eirene, chain, block, signerKey, init.genesis.Config.Eirene, nil, harmoniaSpan.ValidatorSet.Validators)

	_, err := chain.InsertChain([]*types.Block{block})
	require.Equal(t,
		*err.(*eirene.UnauthorizedSignerError),
		eirene.UnauthorizedSignerError{Number: 0, Signer: newAddr.Bytes()})
}
...
func TestEIP1559Transition(t *testing.T) {
	var (
		...
		gspec = &core.Genesis{
			Config: params.EireneUnittestChainConfig,
	...
	actual = state.GetBalance(common.HexToAddress(params.EireneUnittestChainConfig.Eirene.CalculateBurntContract(block.NumberU64()))).ToBig()
...
func TestBurnContract(t *testing.T) {
	...
	gspec.Config.BerlinBlock = common.Big0
	gspec.Config.LondonBlock = common.Big0
	gspec.Config.Eirene.BurntContract = map[string]string{
	...
	// check burnt contract balance
	actual = state.GetBalance(common.HexToAddress(gspec.Config.Eirene.CalculateBurntContract(block.NumberU64()))).ToBig()
	expected = new(big.Int).Mul(new(big.Int).SetUint64(block.GasUsed()), block.BaseFee())
	if actual.Cmp(expected) != 0 {
		t.Fatalf("burnt contract balance incorrect: expected %d, got %d", expected, actual)
	}
...
func TestBurnContractContractFetch(t *testing.T) {
	t.Parallel()
	config := params.EireneUnittestChainConfig
	config.Eirene.BurntContract = map[string]string{
		"10":  "0x000000000000000000000000000000000000aaab",
		"100": "0x000000000000000000000000000000000000aaad",
	}

	burnContractAddr10 := config.Eirene.CalculateBurntContract(10)
	burnContractAddr11 := config.Eirene.CalculateBurntContract(11)
	burnContractAddr99 := config.Eirene.CalculateBurntContract(99)
	burnContractAddr100 := config.Eirene.CalculateBurntContract(100)
	burnContractAddr101 := config.Eirene.CalculateBurntContract(101)
	...
	burnContractAddr100 = config.Eirene.CalculateBurntContract(100)
	burnContractAddr101 = config.Eirene.CalculateBurntContract(101)
	burnContractAddr999 := config.Eirene.CalculateBurntContract(999)
	burnContractAddr1000 := config.Eirene.CalculateBurntContract(1000)
	burnContractAddr1001 := config.Eirene.CalculateBurntContract(1001)
...
func TestJaipurFork(t *testing.T) {
	...
	for i := uint64(1); i < sprintSize; i++ {
		...
		if block.Number().Uint64() == init.genesis.Config.Eirene.JaipurBlock.Uint64()-1 {
			require.Equal(t, testSealHash(block.Header(), init.genesis.Config.Eirene), eirene.SealHash(block.Header(), init.genesis.Config.Eirene))
		}

		if block.Number().Uint64() == init.genesis.Config.Eirene.JaipurBlock.Uint64() {
			require.Equal(t, testSealHash(block.Header(), init.genesis.Config.Eirene), eirene.SealHash(block.Header(), init.genesis.Config.Eirene))
		}
	}
}
...
func testSealHash(header *types.Header, c *params.EireneConfig) (hash common.Hash) {
...
func testEncodeSigHeader(w io.Writer, header *types.Header, c *params.EireneConfig) {
	enc := []interface{}{
		header.ParentHash,
		header.UncleHash,
		header.Coinbase,
