...
func TestSprintLengths(t *testing.T) {
	...
	assert.Equal(t, testEireneConfig.CalculateSprint(0), uint64(16))
	assert.Equal(t, testEireneConfig.CalculateSprint(8), uint64(4))
	assert.Equal(t, testEireneConfig.CalculateSprint(9), uint64(4))
}
...
func TestProducerDelay(t *testing.T) {
	...
	testEireneConfig := params.TestChainConfig.Eirene
	testEireneConfig.ProducerDelay = map[string]uint64{
		"0": 16,
		"8": 4,
	}
	assert.Equal(t, testEireneConfig.CalculateProducerDelay(0), uint64(16))
	assert.Equal(t, testEireneConfig.CalculateProducerDelay(8), uint64(4))
	assert.Equal(t, testEireneConfig.CalculateProducerDelay(9), uint64(4))
}
...
func InitMinerSprintLength(genesis *core.Genesis, privKey *ecdsa.PrivateKey, withoutHarmonia bool) (*node.Node, *eth.Zenanet, error) {
