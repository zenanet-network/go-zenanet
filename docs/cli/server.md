...
The ```eirene server``` command runs the Bor client.

## Options

- ```eirene.devfakeauthor```: Run miner without validator set authorization [dev mode] : Use with '--bor.withoutharmonia' (default: false)

- ```eirene.harmonia```: URL of Harmonia service (default: http://localhost:1317)

- ```eirene.harmoniagRPC```: Address of Harmonia gRPC service

- ```eirene.logs```: Enables eirene log retrieval (default: false)

- ```eirene.runharmonia```: Run Harmonia service as a child process (default: false)

- ```eirene.runharmoniaargs```: Arguments to pass to Harmonia service

- ```eirene.useharmoniaapp```: Use child Harmonia process to fetch data, Only works when bor.runharmonia is true (default: false)

- ```eirene.withoutharmonia```: Run without Harmonia service (for testing purpose) (default: false)
...
- ```state.scheme```: Scheme to use for storing zenanet state ('hash' or 'path') (default: hash)
