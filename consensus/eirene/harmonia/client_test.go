...
import (
	...
	"github.com/zenanet-network/go-zenanet/common/network"
...
func CreateMockHarmoniaServer(wg *sync.WaitGroup, port int, listener net.Listener, handler *HttpHandlerFake) (*http.Server, error) {
...
// TestFetchCheckpointFromMockHarmonia tests the harmonia client side logic
// to fetch checkpoints (latest for the scope of test) from a mock harmonia server.
// It can be used for debugging purpose (like response fields, marshalling/unmarshalling, etc).
func TestFetchCheckpointFromMockHarmonia(t *testing.T) {
	t.Parallel()
	...
	// Create mock harmonia server and pass handler instance for setting up the routes
	srv, err := CreateMockHarmoniaServer(wg, port, listener, handler)
	require.NoError(t, err, "expect no error in starting mock harmonia server")

	// Create a new harmonia client and use same port for connection
	client := NewHarmoniaClient(fmt.Sprintf("http://localhost:%d", port))
	_, err = client.FetchCheckpoint(context.Background(), -1)
	require.NoError(t, err, "expect no error in fetching checkpoint")

	// Shutdown the server
	err = srv.Shutdown(context.TODO())
	require.NoError(t, err, "expect no error in shutting down mock harmonia server")

	// Wait for `wg.Done()` to be called in the mock server's routine.
	wg.Wait()
}

// TestFetchMilestoneFromMockHarmonia tests the harmonia client side logic
// to fetch milestone from a mock harmonia server.
// It can be used for debugging purpose (like response fields, marshalling/unmarshalling, etc).
func TestFetchMilestoneFromMockHarmonia(t *testing.T) {
	t.Parallel()
...
// TestFetchShutdown tests the harmonia client side logic for context timeout and
// interrupt handling while fetching data from a mock harmonia server.
func TestFetchShutdown(t *testing.T) {
	...
	// Case3 - Testing interrupt: Closing the `closeCh` in harmonia client simulating interrupt. This
...
func TestSpanURL(t *testing.T) {
	...
	url, err := spanURL("http://eirene0", 1)
	if err != nil {
		t.Fatal("got an error", err)
	}

	const expected = "http://eirene0/eirene/span/1"
...
func TestStateSyncURL(t *testing.T) {
	...
	url, err := stateSyncURL("http://eirene0", 10, 100)
	if err != nil {
		t.Fatal("got an error", err)
	}

	const expected = "http://eirene0/clerk/event-record/list?from-id=10&to-time=100&limit=50"
