...
var (
	...
	ErrNotSuccessfulResponse = errors.New("error while fetching data from Harmonia")
	ErrNotInRejectedList     = errors.New("milestoneID doesn't exist in rejected list")
	ErrNotInMilestoneList    = errors.New("milestoneID doesn't exist in Harmonia")
	ErrServiceUnavailable    = errors.New("service unavailable")
)

const (
	harmonialAPIBodyLimit = 128 * 1024 * 1024 // 128 MB
	stateFetchLimit      = 50
	apiHarmoniaTimeout   = 5 * time.Second
...
type SpanResponse struct {
	Height string            `json:"height"`
	Result span.HarmoinaSpan `json:"result"`
}

type HarmoniaClient struct {
	urlString string
	client    http.Client
	closeCh   chan struct{}
}
...
func NewHarmoniaClient(urlString string) *HarmoniaClient {
	return &HarmoniaClient{
		urlString: urlString,
		client: http.Client{
			Timeout: apiHarmoniaTimeout,
		},
...
func (h *HarmoniaClient) StateSyncEvents(ctx context.Context, fromID uint64, to int64) ([]*clerk.EventRecordWithTime, error) {
...
func (h *HarmoniaClient) Span(ctx context.Context, spanID uint64) (*span.HarminaSpan, error) {
...
// FetchCheckpoint fetches the checkpoint from harmonia
func (h *HarmoniaClient) FetchCheckpoint(ctx context.Context, number int64) (*checkpoint.Checkpoint, error) {
...
// FetchMilestone fetches the checkpoint from harmonia
func (h *HarmoniaClient) FetchMilestone(ctx context.Context) (*milestone.Milestone, error) {
...
// FetchCheckpointCount fetches the checkpoint count from harmonia
func (h *HarmoniaClient) FetchCheckpointCount(ctx context.Context) (int64, error) {
...
// FetchMilestoneCount fetches the milestone count from harmonia
func (h *HarmoniaClient) FetchMilestoneCount(ctx context.Context) (int64, error) {
...
// FetchLastNoAckMilestone fetches the last no-ack-milestone from harmonia
func (h *HarmoniaClient) FetchLastNoAckMilestone(ctx context.Context) (string, error) {
...
// FetchNoAckMilestone fetches the last no-ack-milestone from harmonia
func (h *HarmoniaClient) FetchNoAckMilestone(ctx context.Context, milestoneID string) error {
...
// to the given milestone is in process in Harmonia
func (h *HarmoniaClient) FetchMilestoneID(ctx context.Context, milestoneID string) error {
...
// FetchWithRetry returns data from harmonia with retry
func FetchWithRetry[T any](ctx context.Context, client http.Client, url *url.URL, closeCh chan struct{}) (*T, error) {
	...
	// yet in harmonia. E.g. when the hardfork hasn't hit yet but harmonia
	// is upgraded.
	if errors.Is(err, ErrServiceUnavailable) {
		log.Debug("Harmonia service unavailable at the moment", "path", url.Path, "error", err)
		return nil, err
	}
	...
	log.Warn("an error while trying fetching from Harmonia", "path", url.Path, "attempt", attempt, "error", err)
...
retryLoop:
	for {
		log.Info("Retrying again in 5 seconds to fetch data from Harmonia", "path", url.Path, "attempt", attempt)
...
func internalFetch(ctx context.Context, client http.Client, u *url.URL) ([]byte, error) {
	...
	limitedBody := http.MaxBytesReader(nil, res.Body, harmoniaAPIBodyLimit)
...
func internalFetchWithTimeout(ctx context.Context, client http.Client, url *url.URL) ([]byte, error) {
	ctx, cancel := context.WithTimeout(ctx, apiHarmoniaTimeout)
	defer cancel()
