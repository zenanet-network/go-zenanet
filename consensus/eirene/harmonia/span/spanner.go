...
func (c *ChainSpanner) CommitSpan(ctx context.Context, heimdallSpan HarmoniaSpan, state *state.StateDB, header *types.Header, chainContext core.ChainContext) error {
	// get validators bytes
	validators := make([]validset.MinimalVal, 0, len(heimdallSpan.ValidatorSet.Validators))
	...
	// get producers bytes
	producers := make([]validset.MinimalVal, 0, len(heimdallSpan.SelectedProducers))
