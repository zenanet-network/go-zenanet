...
import (
	...
	"github.com/zenanet-network/go-zenanet/common/hexutil" //nolint:typecheck
...
func TestGenesisContractChange(t *testing.T) {
	...
	statedb, err := state.New(genesis.Root(), state.NewDatabase(trieDB, nil), nil)
	require.NoError(t, err)

	chain, err := core.NewBlockChain(db, nil, genspec, nil, b, vm.Config{}, nil, nil, nil)
	require.NoError(t, err)
	...
	addBlock := func(root common.Hash, num int64) (common.Hash, *state.StateDB) {
		...
		statedb, err := state.New(h.Root, state.NewDatabase(trieDB, nil), nil)
		require.NoError(t, err)
...
func TestEncodeSigHeaderJaipur(t *testing.T) {
	...
	// was used for Eirene that did not included the BaseFee. The Jaipur
	...
	hash := SealHash(h, &params.EireneConfig{JaipurBlock: big.NewInt(10)})
	require.Equal(t, hash, hashWithoutBaseFee)

	// Jaipur enabled (Jaipur=0) and BaseFee not set
	hash = SealHash(h, &params.EireneConfig{JaipurBlock: common.Big0})
	require.Equal(t, hash, hashWithoutBaseFee)

	h.BaseFee = big.NewInt(2)

	// Jaipur enabled (Jaipur=Header block) and BaseFee set
	hash = SealHash(h, &params.EireneConfig{JaipurBlock: common.Big1})
