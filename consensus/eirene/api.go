...
func (api *API) GetSnapshot(number *rpc.BlockNumber) (*Snapshot, error) {
	...
	return api.eirene.snapshot(api.chain, header.Number.Uint64(), header.Hash(), nil)
}
...
func (api *API) GetAuthor(blockNrOrHash *rpc.BlockNumberOrHash) (*common.Address, error) {
	...
	author, err := api.eirene.Author(header)
...
func (api *API) GetCurrentValidators() ([]*validset.Validator, error) {
	snap, err := api.GetSnapshot(nil)
	if err != nil {
		return make([]*validset.Validator, 0), err
	}
...
func (api *API) GetRootHash(start uint64, end uint64) (string, error) {
	...
	if start > end || end > currentHeaderNumber {
		return "", &validset.InvalidStartEndBlockError{Start: start, End: end, CurrentHeader: currentHeaderNumber}
	}
