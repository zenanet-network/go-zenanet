...
// Eirene is the zenanet-eirene consensus engine
type Eirene struct {
...
func (c *Eirene) FinalizeAndAssemble(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB, body *types.Body, receipts []*types.Receipt) (*types.Block, error) {
	finalizeCtx, finalizeSpan := tracing.StartSpan(context.Background(), "eirene.FinalizeAndAssemble")
...
func (c *Eirene) FetchAndCommitSpan(
	...
	if c.HarmoniaClient == nil {
		// fixme: move to a new mock or fake and remove c.HarmoniaClient completely
	...
	// check if chain id matches with Harmonia span
...
func (c *Eirene) CommitStates(
	...
	log.Info(
		"Fetching state updates from Harmonia",
		"fromID", from,
