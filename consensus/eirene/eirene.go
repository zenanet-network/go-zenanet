...
import (
	...
	"github.com/zenanet-network/go-zenanet/common/tracing"
	balance_tracing "github.com/zenanet-network/go-zenanet/core/tracing"
...
func BorRLP(header *types.Header, c *params.EireneConfig) []byte {
	b := new(bytes.Buffer)
...
// Eirene is the matic-bor consensus engine
type Eirene struct {
	chainConfig *params.ChainConfig  // Chain config
	config      *params.EireneConfig // Consensus engine configuration parameters for bor consensus
	...
	HarmoniaClient         IHarmoniaClient
...
func (c *Eirene) verifyCascadingFields(chain consensus.ChainHeaderReader, header *types.Header, parents []*types.Header) error {
	...
	if IsSprintStart(number+1, c.config.CalculateSprint(number)) {
		...
		sort.Sort(validset.ValidatorsByAddress(newValidators))

		headerVals, err := validset.ParseValidators(header.GetValidatorBytes(c.chainConfig))
		if err != nil {
			return err
		}
...
func (c *Eirene) snapshot(chain consensus.ChainHeaderReader, number uint64, hash common.Hash, parents []*types.Header) (*Snapshot, error) {
	...
	if c.devFakeAuthor && signer.String() != "0x0000000000000000000000000000000000000000" {
		...
		val := validset.NewValidator(signer, 1000)
		validatorset := validset.NewValidatorSet([]*validset.Validator{val})
...
func (c *Eirene) Prepare(chain consensus.ChainHeaderReader, header *types.Header) error {
	...
	if IsSprintStart(number+1, c.config.CalculateSprint(number)) {
		...
		if c.chainConfig.IsCancun(header.Number, header.Number.Uint64()) {
	...
	} else if c.chainConfig.IsCancun(header.Number, header.Number.Uint64()) {
		blockExtraData := &types.BlockExtraData{
			ValidatorBytes: nil,
			TxDependency:   nil,
		}
...
// rewards given.
func (c *Eirene) Finalize(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB, body *types.Body) {
	var (
...
func (c *Eirene) FinalizeAndAssemble(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB, body *types.Body, receipts []*types.Receipt) (*types.Block, error) {
	...
	// set state sync
	bc := chain.(core.EireneStateSyncer)
	bc.SetStateSync(stateSyncData)
...
func Sign(signFn SignerFn, signer common.Address, header *types.Header, c *params.EireneConfig) error {
	sighash, err := signFn(accounts.Account{Address: signer}, accounts.MimetypeEirene, BorRLP(header, c))
	if err != nil {
		return err
	}
...
func (c *Eirene) FetchAndCommitSpan(
	...
	var harmoniaSpan span.HarmoniaSpan

	if c.HarmoniaClient == nil {
		// fixme: move to a new mock or fake and remove c.HeimdallClient completely
		s, err := c.getNextHarmoniaSpanForTest(ctx, newSpanID, header, chain)
		if err != nil {
			return err
		}
		...
		response, err := c.HarmoniaClient.Span(ctx, newSpanID)
		if err != nil {
			return err
		}

		harmoniaSpan = *response
	}

	// check if chain id matches with Heimdall span
	if harmoniaSpan.ChainID != c.chainConfig.ChainID.String() {
		return fmt.Errorf(
			"chain id proposed span, %s, and bor chain id, %s, doesn't match",
			harmoniaSpan.ChainID,
			c.chainConfig.ChainID,
		)
	}

	return c.spanner.CommitSpan(ctx, harmoniaSpan, state, header, chain)
}
...
func (c *Eirene) CommitStates(
	...
	eventRecords, err := c.HarmoniaClient.StateSyncEvents(ctx, from, to.Unix())
	if err != nil {
...
func (c *Eirene) SetHarmoniaClient(h IHarmoniaClient) {
	c.HarmoniaClient = h
}

func (c *Eirene) GetCurrentValidators(ctx context.Context, headerHash common.Hash, blockNumber uint64) ([]*validset.Validator, error) {
...
func (c *Eirene) getNextHarmoniaSpanForTest(
	ctx context.Context,
	newSpanID uint64,
	header *types.Header,
	chain core.ChainContext,
) (*span.HarmoniaSpan, error) {
	headerNumber := header.Number.Uint64()
	...
	selectedProducers := make([]validset.Validator, len(snap.ValidatorSet.Validators))
	for i, v := range snap.ValidatorSet.Validators {
		selectedProducers[i] = *v
	}

	harmoniaSpan := &span.HarmoniaSpan{
		Span:              *spanBor,
	...
	return harmoniaSpan, nil
}

func validatorContains(a []*validset.Validator, x *validset.Validator) (*validset.Validator, bool) {
	for _, n := range a {
...
func getUpdatedValidatorSet(oldValidatorSet *validset.ValidatorSet, newVals []*validset.Validator) *validset.ValidatorSet {
	v := oldValidatorSet
	oldVals := v.Validators

	changes := make([]*validset.Validator, 0, len(oldVals))
