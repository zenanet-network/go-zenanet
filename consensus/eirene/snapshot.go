...
type Snapshot struct {
	...
	ValidatorSet *validset.ValidatorSet      `json:"validatorSet"` // Validator set at this moment
...
func loadSnapshot(chainConfig *params.ChainConfig, config *params.EireneConfig, sigcache *lru.ARCCache, db ethdb.Database, hash common.Hash) (*Snapshot, error) {
...
func (s *Snapshot) apply(headers []*types.Header) (*Snapshot, error) {
	...
	for _, header := range headers {
		...
		if number >= s.chainConfig.Eirene.CalculateSprint(number) {
			delete(snap.Recents, number-s.chainConfig.Eirene.CalculateSprint(number))
		}

		// Resolve the authorization key and check against signers
		signer, err := ecrecover(header, s.sigcache, s.chainConfig.Eirene)
		if err != nil {
			return nil, err
		}
		...
		if number > 0 && (number+1)%s.chainConfig.Eirene.CalculateSprint(number) == 0 {
			...
			newVals, _ := validset.ParseValidators(validatorBytes)
...
func Difficulty(validatorSet *validset.ValidatorSet, signer common.Address) uint64 {
	// if signer is empty
