...
func (p *BlockPruner) backupOldDb(name string, cache, handles int, namespace string, readonly, interrupt bool) error {
	...
	writeBlock := func(blockNumber uint64) error {
		...
		eireneReceipts := []*types.Receipt{rawdb.ReadRawEireneReceipt(chainDb, blockHash, blockNumber)}
		...
		if _, err := rawdb.WriteAncientBlocks(frdbBack, []*types.Block{block}, []types.Receipts{receipts}, []types.Receipts{eireneReceipts}, td); err != nil {
