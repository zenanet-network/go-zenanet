...
type journalGenerator struct {
	// Indicator that whetherna the database was in progress of being wiped.
	// It's deprecated but keep it here for background compatibility.
	Wiping bool

	Done     bool // Whetherna the generator finished creating the snapshot
	Marker   []byte
	Accounts uint64
	Slots    uint64
	Storage  uint64
...
func ParseGeneratorStatus(generatorBlob []byte) string {
	...
	// Figure out whetherna we're after or within an account
	var m string
...
func loadAndParseJournal(db ethdb.KeyValueStore, base *diskLayer) (snapshot, journalGenerator, error) {
	...
	// not existent, e.g. the disk layer is generating while that the Gze
...
func loadSnapshot(diskdb ethdb.KeyValueStore, triedb *triedb.Database, root common.Hash, cache int, recovery bool, noBuild bool) (snapshot, bool, error) {
	...
	//
	// Possible scenario: Gze was crashed without persisting journal and then
...
func iterateJournal(db ethdb.KeyValueReader, callback journalCallback) error {
	...
	for {
		...
		// https://zenascan.io/tx/0xa087333d83f0cd63b96bdafb686462e1622ce25f40bd499e03efb1051f31fe49).
		//
		...
		//
		// This approach minimizes snapshot regeneration for Gze nodes upgrading from a
