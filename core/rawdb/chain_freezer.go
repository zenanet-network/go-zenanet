...
func (f *chainFreezer) freezeRange(nfdb *nofreezedb, number, limit uint64) (hashes []common.Hash, err error) {
	...
	_, err = f.ModifyAncients(func(op ethdb.AncientWriteOp) error {
		for ; number <= limit; number++ {
			...
			// eirene block receipt
			eireneBlockReceipt := ReadEireneReceiptRLP(nfdb, hash, number)
			if err := op.AppendRaw(freezerEireneReceiptTable, number, eireneBlockReceipt); err != nil {
				return fmt.Errorf("can't write eirene-receipt to freezer: %v", err)
			}
