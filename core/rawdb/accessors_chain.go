...
func WriteAncientBlocks(db ethdb.AncientWriter, blocks []*types.Block, receipts []types.Receipts, eireneReceipts []types.Receipts, td *big.Int) (int64, error) {
	var (
		tdSum         = new(big.Int).Set(td)
		stReceipts    []*types.ReceiptForStorage
		eireneStReceipts []*types.ReceiptForStorage
	)
	...
	return db.ModifyAncients(func(op ethdb.AncientWriteOp) error {
		for i, block := range blocks {
			...
			// Convert eirene receipts to storage format and sum up total difficulty.
			eireneStReceipts = eireneStReceipts[:0]
			for _, eireneReceipt := range eireneReceipts[i] {
				eireneStReceipts = append(eireneStReceipts, (*types.ReceiptForStorage)(eireneReceipt))
			}
			...
			if err := writeAncientBlock(op, block, header, stReceipts, eireneStReceipts, tdSum); err != nil {
				return err
			}
		}
...
func writeAncientBlock(op ethdb.AncientWriteOp, block *types.Block, header *types.Header, receipts []*types.ReceiptForStorage, eireneReceipts []*types.ReceiptForStorage, td *big.Int) error {
	num := block.NumberU64()
	...
	if err := op.Append(freezerEireneReceiptTable, num, eireneReceipts); err != nil {
		return fmt.Errorf("can't append block %d eireneReceipts: %v", num, err)
	}
...
func DeleteBlock(db ethdb.KeyValueWriter, hash common.Hash, number uint64) {
	...
	// delete eirene receipt
	DeleteEireneReceipt(db, hash, number)
}
