...
import (
	...
	"github.com/zenanet-network/go-zenanet/common/prque"
	...
	"github.com/zenanet-network/go-zenanet/internal/syncx"
...
type BlockChain struct {
	...
	// Eirene related changes
	eireneReceiptsCache *lru.Cache[common.Hash, *types.Receipt] // Cache for the most recent eirene receipt receipts per block
...
func NewBlockChain(db ethdb.Database, cacheConfig *CacheConfig, genesis *Genesis, overrides *ChainOverrides, engine consensus.Engine, vmConfig vm.Config, shouldPreserve func(header *types.Header) bool, txLookupLimit *uint64, checker zenanet.ChainValidator) (*BlockChain, error) {
	...
	bc := &BlockChain{
		...
		eireneReceiptsCache: lru.NewCache[common.Hash, *types.Receipt](receiptsCacheLimit),
	...
	// it in advance.
	// EIRENE - commented out intentionally
...
func NewParallelBlockChain(db ethdb.Database, cacheConfig *CacheConfig, genesis *Genesis, overrides *ChainOverrides, engine consensus.Engine, vmConfig vm.Config, shouldPreserve func(header *types.Header) bool, txLookupLimit *uint64, checker zenanet.ChainValidator, numprocs int) (*BlockChain, error) {
...
func (bc *BlockChain) setHeadBeyondRoot(head uint64, time uint64, root common.Hash, repair bool) (uint64, error) {
	...
	delFn := func(db ethdb.KeyValueWriter, hash common.Hash, num uint64) {
		...
		if num+1 <= frozen {
			...
			rawdb.DeleteEireneReceipt(db, hash, num)
			rawdb.DeleteEireneTxLookupEntry(db, hash, num)
		}
...
func (bc *BlockChain) InsertReceiptChain(blockChain types.Blocks, receiptChain []types.Receipts, ancientLimit uint64) (int, error) {
	...
	writeAncient := func(blockChain types.Blocks, receiptChain []types.Receipts) (int, error) {
		...
		// EIRENE: Retrieve all the eirene receipts and also maintain the array of headers
		// for eirene specific reorg check.
		eireneReceipts := []types.Receipts{}

		var headers []*types.Header

		for _, block := range blockChain {
			eireneReceipts = append(eireneReceipts, []*types.Receipt{bc.GetEireneReceiptByHash(block.Hash())})
			headers = append(headers, block.Header())
		}

		// Write all chain data to ancients.
		td := bc.GetTd(first.Hash(), first.NumberU64())
		writeSize, err := rawdb.WriteAncientBlocks(bc.db, blockChain, receiptChain, eireneReceipts, td)
		if err != nil {
	...
	writeLive := func(blockChain types.Blocks, receiptChain []types.Receipts) (int, error) {
		...
		for i, block := range blockChain {
			// Update the headers for eirene specific reorg check
			headers = append(headers, block.Header())
...
func (bc *BlockChain) writeBlockWithState(block *types.Block, receipts []*types.Receipt, logs []*types.Log, statedb *state.StateDB) ([]*types.Log, error) {
	...
	if len(blockLogs) > 0 {
		...
		if len(blockLogs) > len(logs) {
			...
			// DeriveFieldsForEireneLogs will fill those fields for websocket subscriptions
			types.DeriveFieldsForEireneLogs(stateSyncLogs, block.Hash(), block.NumberU64(), uint(len(receipts)), uint(len(logs)))

			// Write eirene receipt
			rawdb.WriteEireneReceipt(blockBatch, block.Hash(), block.NumberU64(), &types.ReceiptForStorage{
				Status: types.ReceiptStatusSuccessful, // make receipt status successful
				Logs:   stateSyncLogs,
			})

			// Write eirene tx reverse lookup
			rawdb.WriteEireneTxLookupEntry(blockBatch, block.Hash(), block.NumberU64())
		}
	}
...
func (bc *BlockChain) writeBlockAndSetHead(ctx context.Context, block *types.Block, receipts []*types.Receipt, logs []*types.Log, state *state.StateDB, emitHeadEvent bool) (status WriteStatus, err error) {
	...
	if status == CanonStatTy {
		...
		if emitHeadEvent {
			bc.chainHeadFeed.Send(ChainHeadEvent{Block: block})
			// EIRENE state sync feed related changes
...
func (bc *BlockChain) collectLogs(b *types.Block, removed bool) []*types.Log {
	...
	// Append eirene receipt
	eireneReceipt := rawdb.ReadEireneReceipt(bc.db, b.Hash(), b.NumberU64(), bc.chainConfig)
