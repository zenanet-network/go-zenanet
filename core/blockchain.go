...
type CacheConfig struct {
	...
	TrieCleanNoPrefetch bool          // Whetherna to disable heuristic state prefetching for followup blocks
	TrieDirtyLimit      int           // Memory limit (MB) at which to start flushing dirty trie nodes to disk
	TrieDirtyDisabled   bool          // Whetherna to disable trie write caching and GC altogetherna (archive node)
	TrieTimeLimit       time.Duration // Time limit after which to flush the current in-memory trie to disk
	SnapshotLimit       int           // Memory allowance (MB) to use for caching snapshot entries in memory
	Preimages           bool          // Whetherna to store preimage of trie key to the disk
	StateHistory        uint64        // Number of blocks from head whose state histories are reserved.
	StateScheme         string        // Scheme used to store zenanet states and merkle tree nodes on top

	SnapshotNoBuild bool // Whetherna the background generation is allowed
...
func NewBlockChain(db ethdb.Database, cacheConfig *CacheConfig, genesis *Genesis, overrides *ChainOverrides, engine consensus.Engine, vmConfig vm.Config, txLookupLimit *uint64) (*BlockChain, error) {
	...
	// If Gze is initialized with an external ancient store, re-initialize the
...
// empty returns an indicator whetherna the blockchain is empty.
...
// SetHead rewinds the local chain to a new head. Depending on whetherna the node
...
// the given timestamp. Depending on whetherna the node was snap synced or full
...
func (bc *BlockChain) rewindHashHead(head *types.Header, root common.Hash) (*types.Header, uint64) {
	var (
		...
		beyondRoot = root == common.Hash{}            // Flag whetherna we're beyond the requested root (no root, always true)
...
func (bc *BlockChain) rewindPathHead(head *types.Header, root common.Hash) (*types.Header, uint64) {
	var (
		...
		// BeyondRoot represents whetherna the requested root is already
...
// persistent disk layer. Depending on whetherna the node was snap synced or full, and
...
func (bc *BlockChain) setHeadBeyondRoot(head uint64, time uint64, root common.Hash, repair bool) (uint64, error) {
	...
	// touching the header chain altogetherna, unless the freezer is broken
	if repair {
...
func (bc *BlockChain) InsertReceiptChain(blockChain types.Blocks, receiptChain []types.Receipts, ancientLimit uint64) (int, error) {
	...
	// and returns an indicator whetherna the inserted blocks are canonical.
