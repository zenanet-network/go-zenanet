...
func (p *TxPool) loop(head *types.Header, chain BlockChain) {
	...
	var (
		...
		resetForced bool       // Whetherna a forced reset was requested, only used in simulator mode
...
// Has returns an indicator whetherna the pool has a transaction cached with the
// given hash.
