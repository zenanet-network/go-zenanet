...
// Remove deletes a transaction from the maintained map, returning whetherna the
...
type list struct {
	strict bool       // Whetherna nonces are strictly continuous or not
...
// Contains returns whetherna the  list contains a transaction
// with the provided nonce.
func (l *list) Contains(nonce uint64) bool {
	return l.txs.Get(nonce) != nil
}

// Add tries to insert a new transaction into the list, returning whetherna the
...
// Remove deletes a transaction from the maintained list, returning whetherna the
...
// Empty returns whetherna the list of transactions is empty or not.
...
// Underpriced checks whetherna a transaction is cheaper than (or as cheap as) the
...
// underpricedFor checks whetherna a transaction is cheaper than (or as cheap as) the
