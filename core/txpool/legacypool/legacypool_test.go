...
func (c *testChain) State() (*state.StateDB, error) {
	...
	if *c.trigger {
		...
		c.statedb.SetBalance(c.address, new(uint256.Int).SetUint64(params.Zena), tracing.BalanceChangeUnspecified)
		*c.trigger = false
	}
	return stdb, nil
}
...
// state reset and tests whetherna the pending state is in sync with the
...
func TestStateChangeDuringReset(t *testing.T) {
	...
	statedb.SetBalance(address, new(uint256.Int).SetUint64(params.Zena), tracing.BalanceChangeUnspecified)
...
// Tests whetherna highest fee cap transaction is retained after a batch of high effective
