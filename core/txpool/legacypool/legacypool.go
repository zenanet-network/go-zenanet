...
const (
	...
	// to validate whetherna they fit into the pool or not.
...
type Config struct {
	...
	NoLocals  bool             // Whetherna local transaction handling should be disabled
...
// Filter returns whetherna the given transaction can be consumed by the legacy
// pool, specifically, whetherna it is a Legacy, AccessList or Dynamic transaction.
...
// validateTxBasics checks whetherna a transaction is valid according to the consensus
...
// validateTx checks whetherna a transaction is valid according to the consensus
...
// isGapped reports whetherna the given transaction is immediately executable.
...
// and returns whetherna it was inserted or an older was better.
//
// Note, this method assumes the pool lock is held!
...
// Has returns an indicator whetherna txpool has a transaction cached with the
// given hash.
...
// should return the indicator whetherna the iteration needs to be continued.
