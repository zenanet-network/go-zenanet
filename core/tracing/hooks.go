...
const (
	...
	// BalanceIncreaseWithdrawal is zena withdrawn from the beacon chain.
	BalanceIncreaseWithdrawal BalanceChangeReason = 3
	// BalanceIncreaseGenesisBalance is zena allocated at the genesis block.
	...
	// BalanceIncreaseGasReturn is zena returned for unused gas at the end of execution.
	BalanceIncreaseGasReturn BalanceChangeReason = 7

	// DAO fork
	// BalanceIncreaseDaoContract is zena sent to the DAO refund contract.
	BalanceIncreaseDaoContract BalanceChangeReason = 8
	// BalanceDecreaseDaoAccount is zena taken from a DAO account to be moved to the refund contract.
	BalanceDecreaseDaoAccount BalanceChangeReason = 9

	// BalanceChangeTransfer is zena transferred via a call.
	...
	// BalanceDecreaseSelfdestructBurn is zena that is sent to an already self-destructed
