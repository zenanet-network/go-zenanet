...
import (
	...
	"github.com/zenanet-network/go-zenanet/core/asm"
...
func TestCall(t *testing.T) {
	state, _ := state.New(types.EmptyRootHash, state.NewDatabaseForTesting(), nil)
...
func benchmarkNonModifyingCode(gas uint64, code []byte, name string, tracerCode string, b *testing.B) {
	cfg := new(Config)
	setDefaults(cfg)
	cfg.State, _ = state.New(types.EmptyRootHash, state.NewDatabaseForTesting(), nil)
	cfg.GasLimit = gas
	if len(tracerCode) > 0 {
...
// correctly
// see: https://github.com/zenanet-network/go-zenanet/issues/22649
