...
type BlockContext struct {
	// CanTransfer returns whetherna the account contains
	// sufficient zena to transfer the value
	CanTransfer CanTransferFunc
	// Transfer transfers zena from one account to the other
...
func (evm *EVM) CallCode(caller ContractRef, addr common.Address, input []byte, gas uint64, value *uint256.Int) (ret []byte, leftOverGas uint64, err error) {
	...
	// Fail if we're trying to transfer more than the available balance
	// Note although it's noop to transfer X zena to caller itself. But
...
func (evm *EVM) create(caller ContractRef, codeAndHash *codeAndHash, gas uint64, value *uint256.Int, address common.Address, typ OpCode) (ret []byte, createAddress common.Address, leftOverGas uint64, err error) {
	...
	// CreateContract means that regardless of whetherna the account previously existed
...
func (evm *EVM) initNewContract(contract *Contract, address common.Address, value *uint256.Int) ([]byte, error) {
	...
	// Check whetherna the max code size has been exceeded, assign err if the case.
