...
// DAG는 간단한 DAG 구조체 구현입니다.
type DAG struct {
	vertices map[string]int      // vertex id (문자열)와 원래 값(정수) 매핑
	edges    map[string][]string // from vertex id와 연결된 to vertex id 리스트
}

// NewDAG는 새 DAG 인스턴스를 생성합니다.
func NewDAG() *DAG {
	return &DAG{
...
// AddVertex는 새 정점을 추가하고 그에 해당하는 문자열 ID를 반환합니다.
func (d *DAG) AddVertex(v int) (string, error) {
	id := strconv.Itoa(v)
	if _, exists := d.vertices[id]; exists {
		return id, fmt.Errorf("vertex %s already exists", id)
	}
	d.vertices[id] = v
	return id, nil
}

// AddEdge는 from에서 to로 가는 단방향 엣지를 추가합니다.
...
// 기존 코드에서 사용되는 GetVertices, GetParents 등 필요한 메서드도 추가합니다.
func (d *DAG) GetVertices() []interface{} {
	ret := make([]interface{}, 0, len(d.vertices))
	for _, val := range d.vertices {
		ret = append(ret, val)
	}
	return ret
}

func (d *DAG) GetParents(id string) ([]interface{}, error) {
	var parents []interface{}
	for from, children := range d.edges {
