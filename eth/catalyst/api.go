...
import (
	...
	"github.com/zenanet-network/go-zenanet/beacon/engine"
...
type ConsensusAPI struct {
	...
	//     persist any bad block information to disk as a bug in Gze could end
	//     up blocking a valid chain, even if a later Gze update would accept
	//     it.
	//   - Bad blocks will get forgotten after a certain threshold of import
	//     attempts and will be retried. The rationale is that if the network
	//     really-really-really tries to feed us a block, we should give it a
	//     new chance, perhaps us being racey instead of the block being legit
	//     bad (this happened in Gze at a point with import vs. pending race).
	...
	// Gze can appear to be stuck or do strange things if the beacon client is
...
func (api *ConsensusAPI) forkchoiceUpdated(update engine.ForkchoiceStateV1, payloadAttributes *engine.PayloadAttributes, payloadVersion engine.PayloadVersion, payloadWitness bool) (engine.ForkChoiceResponse, error) {
	...
	// Check whetherna we have the block yet in our database or not. If not, we'll
...
func (api *ConsensusAPI) ExchangeTransitionConfigurationV1(config engine.TransitionConfigurationV1) (*engine.TransitionConfigurationV1, error) {
	...
	if ttd == nil || ttd.Cmp(config.TerminalTotalDifficulty.ToInt()) != 0 {
		log.Warn("Invalid TTD configured", "gze", ttd, "beacon", config.TerminalTotalDifficulty)
...
func (api *ConsensusAPI) newPayload(params engine.ExecutableData, versionedHashes []common.Hash, beaconRoot *common.Hash, requests [][]byte, witness bool) (engine.PayloadStatusV1, error) {
	...
	// check whetherna we already have the block locally.
...
// checkInvalidAncestor checks whetherna the specified chain end links to a known
