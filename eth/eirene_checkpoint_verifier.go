...
func newEireneVerifier() *eireneVerifier {
	return &eireneVerifier{eireneVerify}
}

func eireneVerify(ctx context.Context, eth *Zenanet, handler *ethHandler, start uint64, end uint64, hash string, isCheckpoint bool) (string, error) {
	str := "milestone"
	if isCheckpoint {
		str = "checkpoint"
	}
