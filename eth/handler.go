...
import (
	...
	"github.com/zenanet-network/go-zenanet/core/forkid"
	...
	"github.com/zenanet-network/go-zenanet/eth/fetcher"
...
type txPool interface {
	// Has returns an indicator whetherna txpool has a transaction
	...
	// can decide whetherna to receive notifications only for newly seen transactions
...
type handlerConfig struct {
	...
	Sync           ethconfig.SyncMode     // Whetherna to snap or full sync
...
type handler struct {
	...
	snapSync atomic.Bool // Flag whetherna snap sync is enabled (gets disabled if we already have blocks)
	synced   atomic.Bool // Flag whetherna we're considered synchronised (enables transaction processing)
...
func (h *handler) BroadcastTransactions(txs types.Transactions) {
	...
	for _, tx := range txs {
		...
		//
		// To do this, we hash the local enode IW with togetherna with a peer's
		// enode ID togetherna with the transaction sender and broadcast if
