...
func NewFilterAPI(system *FilterSystem, eireneLogs bool) *FilterAPI {
	api := &FilterAPI{
		sys:     system,
		events:  NewEventSystem(system),
		filters: make(map[rpc.ID]*filter),
		timeout: system.cfg.Timeout,
		eireneLogs: eireneLogs,
	}
...
func (api *FilterAPI) GetLogs(ctx context.Context, crit FilterCriteria) ([]*types.Log, error) {
	...
	eireneConfig := api.sys.backend.ChainConfig().Eirene

	var filter *Filter

	var eireneLogsFilter *EireneBlockLogsFilter
	...
	if crit.BlockHash != nil {
		...
		// Block eirene filter
		if api.eireneLogs {
			eireneLogsFilter = NewEireneBlockLogsFilter(api.sys.backend, eireneConfig, *crit.BlockHash, crit.Addresses, crit.Topics)
		}
	} else {
		...
		// Block eirene filter
		if api.eireneLogs {
			eireneLogsFilter = NewEireneBlockLogsRangeFilter(api.sys.backend, eireneConfig, begin, end, crit.Addresses, crit.Topics)
		}
	}
	...
	if eireneLogsFilter != nil {
		// Run the filter and return all the logs
		eireneBlockLogs, err := eireneLogsFilter.Logs(ctx)
		if err != nil {
			return nil, err
		}

		return returnLogs(types.MergeEireneLogs(logs, eireneBlockLogs)), err
	}

	// merge eirene block logs and receipt logs and return it
...
func (api *FilterAPI) GetFilterLogs(ctx context.Context, id rpc.ID) ([]*types.Log, error) {
	...
	if f.crit.BlockHash != nil {
		...
		// Block eirene filter
		if api.eireneLogs {
			eireneLogsFilter = NewEireneBlockLogsFilter(api.sys.backend, eireneConfig, *f.crit.BlockHash, f.crit.Addresses, f.crit.Topics)
		}
	} else {
		// Convert the RPC block numbers into internal representations
		begin := rpc.LatestBlockNumber.Int64()
		...
		if api.eireneLogs {
			eireneLogsFilter = NewEireneBlockLogsRangeFilter(api.sys.backend, eireneConfig, begin, end, f.crit.Addresses, f.crit.Topics)
		}
	}
	// Run the filter and return all the logs
	logs, err := filter.Logs(ctx)
	...
	if eireneLogsFilter != nil {
		...
		return returnLogs(types.MergeEireneLogs(logs, eireneBlockLogs)), nil
	}
