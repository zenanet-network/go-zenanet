...
func (api *API) traceEireneBlock(ctx context.Context, block *types.Block, config *TraceConfig) (*BlockTraceResult, error) {
	...
	traceTxn := func(indx int, tx *types.Transaction, eireneTx bool, stateSyncHash common.Hash) *TxTraceResult {
		...
		if eireneTx {
			...
			execRes, err = statefull.ApplyEireneMessage(vmenv, callmsg)
		} else {
...
func (api *API) TraceEireneBlock(req *TraceBlockRequest) (*BlockTraceResult, error) {
	...
	log.Debug("Tracing Eirene Block", "block number", blockNumber)
	...
	return api.traceEireneBlock(ctx, block, req.Config)
}
