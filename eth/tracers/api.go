...
var defaultEireneTraceEnabled = newBoolPtr(false)
...
func (api *API) getAllBlockTransactions(ctx context.Context, block *types.Block) (types.Transactions, bool, common.Hash) {
	...
	eireneReceipt := rawdb.ReadEireneReceipt(api.backend.ChainDb(), block.Hash(), block.NumberU64(), api.backend.ChainConfig())
	if eireneReceipt != nil {
		txHash := types.GetDerivedEireneTxHash(types.EireneReceiptKey(block.Number().Uint64(), block.Hash()))
		if txHash != (common.Hash{}) {
			eireneTx, _, _, _, _ := api.backend.GetEireneBlockTransactionWithBlockHash(ctx, txHash, block.Hash())
...
func (api *API) traceChain(start, end *types.Block, config *TraceConfig, closed <-chan error) chan *blockTraceResult {
	if config == nil {
		config = &TraceConfig{
			EireneTraceEnabled: defaultEireneTraceEnabled,
			EireneTx:           newBoolPtr(false),
		}
	}

	if config.EireneTraceEnabled == nil {
		config.EireneTraceEnabled = defaultEireneTraceEnabled
	}
	...
	for th := 0; th < threads; th++ {
		...
		go func() {
			...
			for task := range taskCh {
				...
				if !*config.EireneTraceEnabled && stateSyncPresent {
					txs = txs[:len(txs)-1]
					stateSyncPresent = false
				}
				...
				for i, tx := range txs {
					...
					if stateSyncPresent && i == len(txs)-1 {
						if *config.EireneTraceEnabled {
							config.EireneTx = newBoolPtr(true)
						}
					}
	...
	for th := 0; th < threads; th++ {
		...
		go func() {
			...
			for task := range jobs {
				...
				// concurrent use.
				// See: https://github.com/zenanet-network/go-zenanet/issues/29114
		...
		if !ioflag {
			...
			if stateSyncPresent && i == len(txs)-1 {
				if *config.EireneTraceEnabled {
					callmsg := prepareCallMessage(*msg)
					// nolint : contextcheck
					if _, err := statefull.ApplyEireneMessage(vmenv, callmsg); err != nil {
						failed = err
						break txloop
					}
				} else {
	...
	for i, tx := range block.Transactions() {
		...
		if stateSyncPresent && i == len(txs)-1 {
			if *config.EireneTraceEnabled {
				callmsg := prepareCallMessage(*msg)
				statedb.SetTxContext(stateSyncHash, i)
				_, err = statefull.ApplyEireneMessage(vmenv, callmsg)
...
func (api *API) TraceTransaction(ctx context.Context, hash common.Hash, config *TraceConfig) (interface{}, error) {
	...
	if !found {
		// For EireneTransaction, there will be no trace available
		tx, _, _, _ := rawdb.ReadEireneTransaction(api.backend.ChainDb(), hash)
		if tx != nil {
...
func (api *API) traceTx(ctx context.Context, tx *types.Transaction, message *core.Message, txctx *Context, vmctx vm.BlockContext, statedb *state.StateDB, config *TraceConfig) (interface{}, error) {
	...
	if config.EireneTx == nil {
		config.EireneTx = newBoolPtr(false)
	}
