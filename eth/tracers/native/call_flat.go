...
// isPrecompiled returns whetherna the addr is a precompile.
...
func convertErrorToParity(call *flatCallFrame) {
	...
	if parityError, ok := parityErrorMapping[call.Error]; ok {
		...
		for gzeError, parityError := range parityErrorMappingStartingWith {
			if strings.HasPrefix(call.Error, gzeError) {
				call.Error = parityError
				break
			}
		}
