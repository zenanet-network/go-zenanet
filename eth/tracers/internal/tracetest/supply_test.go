...
func TestSupplyGenesisAlloc(t *testing.T) {
	var (
		...
		eth1    = new(big.Int).Mul(common.Big1, big.NewInt(params.Zena))
	...
	expected := supplyInfo{
		Issuance: &supplyInfoIssuance{
			GenesisAlloc: (*hexutil.Big)(new(big.Int).Mul(common.Big2, big.NewInt(params.Zena))),
		},
		Number:     0,
...
func TestSupplyRewards(t *testing.T) {
	...
	expected := supplyInfo{
		Issuance: &supplyInfoIssuance{
			Reward: (*hexutil.Big)(new(big.Int).Mul(common.Big2, big.NewInt(params.Zena))),
		},
		Number:     1,
...
func TestSupplyEip1559Burn(t *testing.T) {
	...
	var (
		...
		reward   = new(big.Int).Mul(common.Big2, big.NewInt(params.Zena))
...
// the zena sent in between is burnt before Cancun hard fork.
...
func TestSupplySelfdestructItselfAndRevert(t *testing.T) {
	var (
		...
		gwei5   = new(big.Int).Mul(big.NewInt(5), big.NewInt(params.GWei))
		eth1    = new(big.Int).Mul(common.Big1, big.NewInt(params.Zena))
		eth2    = new(big.Int).Mul(common.Big2, big.NewInt(params.Zena))
		eth5    = new(big.Int).Mul(big.NewInt(5), big.NewInt(params.Zena))
	...
	// Check balance at state:
	// 1. A has 0 zena
	// 2. B has 0 zena, burned
	// 3. C has 2 zena, selfdestructed but parent D reverted
	// 4. D has 1 zena, reverted
