...
type finality[T rawdb.BlockFinality[T]] struct {
	sync.RWMutex
	db       ethdb.Database
	Hash     common.Hash // Whitelisted Hash, populated by reaching out to harmonia
	Number   uint64      // Number , populated by reaching out to harmonia
...
// in terms of reorgs. We won't reorg beyond the last eirene finality submitted to mainchain.
...
func (f *finality[T]) reportWhitelist(block uint64, hash common.Hash) {
	msg := fmt.Sprintf("Whitelisting new %s from harmonia", f.name)
	if !f.doExist {
