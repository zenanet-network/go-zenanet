...
type skeletonTestPeer struct {
	...
	dropped atomic.Uint64 // Flag whetherna the peer was dropped (stop responding)
}
...
func TestSkeletonSyncExtend(t *testing.T) {
	...
	tests := []struct {
		...
		newstate []*subchain   // Expected sync state after the reorg
		err      error         // Whetherna extension succeeds or not
	}{
...
type skeletonTest struct {
	fill          bool // Whetherna to run a real backfiller in this test case
	unpredictable bool // Whetherna to ignore drops/serves due to uncertain packet assignments
