...
import (
	...
	"github.com/zenanet-network/go-zenanet/consensus/clique"
	...
	"github.com/zenanet-network/go-zenanet/core/bloombits"
	"github.com/zenanet-network/go-zenanet/core/rawdb"
	"github.com/zenanet-network/go-zenanet/core/state/pruner"
	...
	"github.com/zenanet-network/go-zenanet/eth/downloader/whitelist"
	"github.com/zenanet-network/go-zenanet/eth/ethconfig"
	"github.com/zenanet-network/go-zenanet/eth/filters"
	"github.com/zenanet-network/go-zenanet/eth/gasprice"
	"github.com/zenanet-network/go-zenanet/eth/protocols/eth"
	...
	"github.com/zenanet-network/go-zenanet/internal/shutdowncheck"
	...
	"github.com/zenanet-network/go-zenanet/p2p/dnsdisc"
...
func New(stack *node.Node, config *ethconfig.Config) (*Zenanet, error) {
	...
	if config.Miner.GasPrice == nil || config.Miner.GasPrice.Cmp(big.NewInt(params.EireneDefaultMinerGasPrice)) != 0 {
	...
	// EIRENE changes
	// Blob pool is removed from Subpool for Eirene
	...
	eth.txPool.SetGasTip(new(big.Int).SetUint64(params.EireneDefaultTxPoolPriceLimit))
	...
	return append(apis, []rpc.API{
		{
			...
			Namespace: "eth",
			Service:   publicFilterAPI, // EIRENE related change
		}, {
			Namespace: "admin",
...
func (s *Zenanet) StartMining() error {
	...
	if !s.IsMining() {
		...
		if !s.authorized {
			...
			if eirene, ok := s.engine.(*eirene.Eirene); ok {
				wallet, err := s.accountManager.Find(accounts.Account{Address: eb})
				if wallet == nil || err != nil {
					log.Error("Zenbase account unavailable locally", "err", err)
				...
				eirene.Authorize(eb, wallet.SignData)
			}
		}
...
var (
	ErrNotEireneConsensus             = errors.New("not eirene consensus was given")
	ErrEireneConsensusWithoutHarmonia = errors.New("eirene consensus without harmonia")
)
...
func (s *Zenanet) startCheckpointWhitelistService() {
	...
	s.retryHarmoniaHandler(s.handleWhitelistCheckpoint, tickerDuration, whitelistTimeout, fnName)
}
...
func (s *Zenanet) startMilestoneWhitelistService() {
	...
	s.retryHarmoniaHandler(s.handleMilestone, tickerDuration, whitelistTimeout, fnName)
}
...
func (s *Zenanet) startNoAckMilestoneService() {
	...
	s.retryHarmoniaHandler(s.handleNoAckMilestone, tickerDuration, noAckMilestoneTimeout, fnName)
}
...
func (s *Zenanet) startNoAckMilestoneByIDService() {
	...
	s.retryHarmoniaHandler(s.handleNoAckMilestoneByID, tickerDuration, noAckMilestoneTimeout, fnName)
}

func (s *Zenanet) retryHarmoniaHandler(fn harmoniaHandler, tickerDuration time.Duration, timeout time.Duration, fnName string) {
	retryHarmoniaHandler(fn, tickerDuration, timeout, fnName, s.closeCh, s.getHandler)
}

func retryHarmoniaHandler(fn harmoniaHandler, tickerDuration time.Duration, timeout time.Duration, fnName string, closeCh chan struct{}, getHandler func() (*ethHandler, *eirene.Eirene, error)) {
	...
	ethHandler, eirene, err := getHandler()
	if err != nil {
	...
	_ = fn(firstCtx, ethHandler, eirene)
...
func (s *Zenanet) handleWhitelistCheckpoint(ctx context.Context, ethHandler *ethHandler, eirene *eirene.Eirene) error {
	// Create a new eirene verifier, which will be used to verify checkpoints and milestones
	verifier := newEireneVerifier()

	blockNum, blockHash, err := ethHandler.fetchWhitelistCheckpoint(ctx, eirene, s, verifier)
...
type harmoniaHandler func(ctx context.Context, ethHandler *ethHandler, eirene *eirene.Eirene) error

// handleMilestone handles the milestone mechanism.
func (s *Zenanet) handleMilestone(ctx context.Context, ethHandler *ethHandler, eirene *eirene.Eirene) error {
	// Create a new eirene verifier, which will be used to verify checkpoints and milestones
	verifier := newEireneVerifier()
	num, hash, err := ethHandler.fetchWhitelistMilestone(ctx, eirene, s, verifier)
	...
	if errors.Is(err, harmonia.ErrServiceUnavailable) {
		return nil
	}
...
func (s *Zenanet) handleNoAckMilestone(ctx context.Context, ethHandler *ethHandler, eirene *eirene.Eirene) error {
	milestoneID, err := ethHandler.fetchNoAckMilestone(ctx, eirene)
...
func (s *Zenanet) handleNoAckMilestoneByID(ctx context.Context, ethHandler *ethHandler, eirene *eirene.Eirene) error {
	milestoneIDs := ethHandler.downloader.GetMilestoneIDsList()

	for _, milestoneID := range milestoneIDs {
		// todo: check if we can ignore the error
		err := ethHandler.fetchNoAckMilestoneByID(ctx, eirene, milestoneID)
		if err == nil {
			ethHandler.downloader.RemoveMilestoneID(milestoneID)
		}
	}
...
func (s *Zenanet) getHandler() (*ethHandler, *eirene.Eirene, error) {
	ethHandler := (*ethHandler)(s.handler)

	eirene, ok := ethHandler.chain.Engine().(*eirene.Eirene)
	if !ok {
		return nil, nil, ErrNotEireneConsensus
	}

	if eirene.HarmoniaClient == nil {
		return nil, nil, ErrEireneConsensusWithoutHarmonia
	}

	return ethHandler, eirene, nil
}
...
func (s *Zenanet) Stop() error {
	...
	// the harmonia is unresponsive and the syncing loop keeps waiting
