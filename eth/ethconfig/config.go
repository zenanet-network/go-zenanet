...
type Config struct {
	...
	// State options.
	NoPruning  bool // Whetherna to disable pruning and flush everything to disk
	NoPrefetch bool // Whetherna to disable prefetching and only load state on demand
	...
	// canonical chain of all remote peers. Setting the option makes gze verify the
	...
	// send-transaction variants. The unit is zena.
	RPCTxFeeCap float64
...
func CreateConsensusEngine(config *params.ChainConfig, db ethdb.Database) (consensus.Engine, error) {
	if config.TerminalTotalDifficulty == nil {
		log.Error("Gze only supports PoS networks. Please transition legacy networks using Gze v1.13.x.")
