...
import (
	...
	"github.com/zenanet-network/go-zenanet/consensus/eirene/contract" //nolint:typecheck
...
type Config struct {
	...
	// URL to connect to Harmonia node
	HarmoniaURL string

	// No harmonia service
	WithoutHarmonia bool

	// Address to connect to Harmonia gRPC server
	HarmoniagRPCAddress string

	// Run harmonia service as a child process
	RunHarmonia bool

	// Arguments to pass to harmonia service
	RunHarmoniaArgs string

	// Use child harmonia process to fetch data, Only works when RunHarmonia is true
	UseHarmoniaApp bool
...
func CreateConsensusEngine(chainConfig *params.ChainConfig, ethConfig *Config, db ethdb.Database, blockchainAPI *ethapi.BlockChainAPI) (consensus.Engine, error) {
	...
	if chainConfig.Clique != nil {
	...
	} else if chainConfig.Eirene != nil && chainConfig.Eirene.ValidatorContract != "" {
		// If Matic eirene consensus is requested, set it up
		...
		genesisContractsClient := contract.NewGenesisContractsClient(chainConfig, chainConfig.Eirene.ValidatorContract, chainConfig.Eirene.StateReceiverContract, blockchainAPI)
		spanner := span.NewChainSpanner(blockchainAPI, contract.ValidatorSet(), chainConfig, common.HexToAddress(chainConfig.Eirene.ValidatorContract))

		if ethConfig.WithoutHarmonia {
			...
			if ethConfig.DevFakeAuthor {
				log.Warn("Sanitizing DevFakeAuthor", "Use DevFakeAuthor with", "--eirene.withoutharmonia")
			}

			var harmoniaClient eirene.IHarmoniaClient
			if ethConfig.RunHarmonia && ethConfig.UseHarmoniaApp {
				harmoniaClient = harmoniaapp.NewHarmoniaAppClient()
			} else if ethConfig.HarmoniagRPCAddress != "" {
				harmoniaClient = harmoniagrpc.NewHarmoniaGRPCClient(ethConfig.HarmoniagRPCAddress)
			} else {
				harmoniaClient = harmonia.NewHarmoniaClient(ethConfig.HarmoniaURL)
			}

			return eirene.New(chainConfig, db, blockchainAPI, spanner, harmoniaClient, genesisContractsClient, false), nil
		}
	}
