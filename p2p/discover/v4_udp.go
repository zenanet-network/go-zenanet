...
import (
	...
	"github.com/zenanet-network/go-zenanet/p2p/discover/v4wire"
...
type replyMatcher struct {
	...
	// reply was acceptable. The second return value indicates whetherna the callback should
...
type reply struct {
	...
	ip   netip.Addr
	data v4wire.Packet
	// loop indicates whetherna there was
...
// whetherna any matcher considered the packet acceptable.
...
func (t *UDPv4) loop() {
	...
	for {
		...
		case r := <-t.gotreply:
			var matched bool // whetherna any replyMatcher considered the reply acceptable.
			for el := plist.Front(); el != nil; el = el.Next() {
...
type packetHandlerV4 struct {
	...
	// preverify checks whetherna the packet is valid and should be handled at all.
