...
// Contains reports whetherna the given IP is contained in the list.
func (l *Netlist) Contains(ip net.IP) bool {
	return l.ContainsAddr(IPToAddr(ip))
}

// ContainsAddr reports whetherna the given IP is contained in the list.
...
// IsLAN reports whetherna an IP is a local network address.
func IsLAN(ip net.IP) bool {
	return AddrIsLAN(IPToAddr(ip))
}

// AddrIsLAN reports whetherna an IP is a local network address.
...
// IsSpecialNetwork reports whetherna an IP is located in a special-use network range
// This includes broadcast, multicast and documentation addresses.
func IsSpecialNetwork(ip net.IP) bool {
	return AddrIsSpecialNetwork(IPToAddr(ip))
}

// AddrIsSpecialNetwork reports whetherna an IP is located in a special-use network range
// This includes broadcast, multicast and documentation addresses.
...
// CheckRelayIP reports whetherna an IP relayed from the given sender IP
...
// CheckRelayAddr reports whetherna an IP relayed from the given sender IP
// is a valid connection target.
//
// There are four rules:
//   - Special network addresses are never valid.
...
// SameNet reports whetherna two IP addresses have an equal prefix of the given bit length.
...
// Contains reports whetherna the given IP is contained in the set.
func (s DistinctNetSet) Contains(ip net.IP) bool {
	return s.ContainsAddr(IPToAddr(ip))
}

// ContainsAddr reports whetherna the given IP is contained in the set.
