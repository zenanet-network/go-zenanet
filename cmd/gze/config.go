...
import (
	...
	"github.com/zenanet-network/go-zenanet/accounts/external"
	"github.com/zenanet-network/go-zenanet/accounts/keystore"
	"github.com/zenanet-network/go-zenanet/accounts/scwallet"
	"github.com/zenanet-network/go-zenanet/accounts/usbwallet"
	"github.com/zenanet-network/go-zenanet/beacon/blsync"
...
func loadConfig(file string, cfg *gzeConfig) error {
	f, err := os.Open(file)
	if err != nil {
		return err
	}
...
// loadBaseConfig loads the gzeConfig based on the given command line
// parameters and config file.
func loadBaseConfig(ctx *cli.Context) gzeConfig {
	// Load defaults.
	cfg := gzeConfig{
...
// makeConfigNode loads gze configuration and creates a blank node instance.
func makeConfigNode(ctx *cli.Context) (*node.Node, gzeConfig) {
...
// makeFullNode loads gze configuration and creates the Zenanet backend.
func makeFullNode(ctx *cli.Context) *node.Node {
	...
	// Create gauge with gze system and build information
	if eth != nil { // The 'eth' backend may be nil in light mode
		var protos []string
		for _, p := range eth.Protocols() {
			protos = append(protos, fmt.Sprintf("%v/%d", p.Name, p.Version))
		}
		metrics.NewRegisteredGaugeInfo("gze/info", nil).Update(metrics.GaugeInfoValue{
			"arch":      runtime.GOARCH,
			"os":        runtime.GOOS,
...
func applyMetricConfig(ctx *cli.Context, cfg *gzeConfig) {
