...
import (
	...
	"github.com/zenanet-network/go-zenanet/console"
...
var (
	...
	consoleCommand = &cli.Command{
		...
		Description: `
The Gze console is an interactive shell for the JavaScript runtime environment
which exposes a node admin interface as well as the Ðapp JavaScript API.
See https://gze.ethereum.org/docs/interacting-with-gze/javascript-console.`,
	}
...
which exposes a node admin interface as well as the Ðapp JavaScript API.
See https://gze.ethereum.org/docs/interacting-with-gze/javascript-console.
This command allows to open a console on a running gze node.`,
	}
...
JavaScript API. See https://gze.ethereum.org/docs/interacting-with-gze/javascript-console`,
	}
)

// localConsole starts a new gze node, attaching a JavaScript console to it at the
// same time.
...
// remoteConsole will connect to a remote gze instance, attaching a JavaScript
// console to it.
func remoteConsole(ctx *cli.Context) error {
	...
	if err != nil {
		utils.Fatalf("Unable to attach to remote gze: %v", err)
	}
	config := console.Config{
		DataDir: utils.MakeDataDir(ctx),
		DocRoot: ctx.String(utils.JSpathFlag.Name),
...
// ephemeralConsole starts a new gze node, attaches an ephemeral JavaScript
