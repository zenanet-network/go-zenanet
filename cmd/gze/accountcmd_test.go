...
`
	...
	{
		gze := runGze(t, "account", "list", "--datadir", datadir)
		gze.Expect(want)
		gze.ExpectExit()
	}
	{
		gze := runGze(t, "--datadir", datadir, "account", "list")
		gze.Expect(want)
		gze.ExpectExit()
	}
}
...
func TestAccountNew(t *testing.T) {
	t.Parallel()
	gze := runGze(t, "account", "new", "--lightkdf")
	defer gze.ExpectExit()
	gze.Expect(`
...
func TestAccountHelp(t *testing.T) {
	t.Parallel()
	gze := runGze(t, "account", "-h")
	gze.WaitExit()
	if have, want := gze.ExitStatus(), 0; have != want {
		t.Errorf("exit error, have %d want %d", have, want)
	}

	gze = runGze(t, "account", "import", "-h")
	gze.WaitExit()
	if have, want := gze.ExitStatus(), 0; have != want {
		t.Errorf("exit error, have %d want %d", have, want)
	}
...
func importAccountWithExpect(t *testing.T, key string, expected string) {
	...
	gze := runGze(t, "--lightkdf", "account", "import", "-password", passwordFile, keyfile)
	defer gze.ExpectExit()
	gze.Expect(expected)
}
...
func TestWalletImport(t *testing.T) {
	t.Parallel()
	gze := runGze(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer gze.ExpectExit()
	gze.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := os.ReadDir(filepath.Join(gze.Datadir, "keystore"))
	if len(files) != 1 {
