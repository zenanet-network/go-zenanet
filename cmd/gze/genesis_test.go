...
// Tests that initializing Gze with a custom genesis block and chain definitions
// work properly.
func TestCustomGenesis(t *testing.T) {
	...
	for i, tt := range customGenesisTests {
		...
		runGze(t, "--datadir", datadir, "init", json).WaitExit()

		// Query the custom genesis block
		gze := runGze(t, "--networkid", "1337", "--syncmode=full", "--cache", "16",
			"--datadir", datadir, "--maxpeers", "0", "--port", "0", "--authrpc.port", "0",
			"--nodiscover", "--nat", "none", "--ipcdisable",
			"--exec", tt.query, "console")
		gze.ExpectRegexp(tt.result)
		gze.ExpectExit()
	}
}
...
func TestCustomBackend(t *testing.T) {
	...
	testfunc := func(t *testing.T, tt backendTest) error {
		...
		{ // Init
			...
			gze := runGze(t, args...)
			gze.ExpectRegexp(tt.initExpect)
			gze.ExpectExit()
		}
		{ // Exec + query
			...
			gze := runGze(t, args...)
			gze.ExpectRegexp(tt.execExpect)
			gze.ExpectExit()
		}
		return nil
	}
