...
// gze is a command-line client for Zenanet.
package main
...
const (
	clientIdentifier = "gze" // Client identifier to advertise over the network
)
...
var (
	...
	nodeFlags = slices.Concat([]cli.Flag{
		...
		utils.MinerZenabaseFlag, // deprecated
...
func init() {
	// Initialize the CLI app and start Gze
	app.Action = gze
	...
	flags.AutoEnvVars(app.Flags, "GZE")
	...
	app.Before = func(ctx *cli.Context) error {
		...
		flags.CheckEnvVars(ctx, app.Flags, "GZE")
		return nil
	}
...
func prepare(ctx *cli.Context) {
	...
	case ctx.IsSet(utils.SepoliaFlag.Name):
		log.Info("Starting Gze on Sepolia testnet...")

	case ctx.IsSet(utils.HoleskyFlag.Name):
		log.Info("Starting Gze on Holesky testnet...")

	case ctx.IsSet(utils.DeveloperFlag.Name):
		log.Info("Starting Gze in ephemeral dev mode...")
		log.Warn(`You are running Gze in --dev mode. Please note the following:
...
`)
	...
	case !ctx.IsSet(utils.NetworkIdFlag.Name):
		log.Info("Starting Gze on Zenanet mainnet...")
	}
...
// gze is the main entry point into the system if no special subcommand is run.
// It creates a default node based on the command line arguments and runs it in
// blocking mode, waiting for it to be shut down.
func gze(ctx *cli.Context) error {
...
func startNode(ctx *cli.Context, stack *node.Node, isConsole bool) {
	...
	// Create a client to interact with local gze node.
