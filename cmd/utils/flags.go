...
import (
	...
	bparams "github.com/zenanet-network/go-zenanet/beacon/params"
	...
	"github.com/zenanet-network/go-zenanet/core/txpool/blobpool"
	...
	"github.com/zenanet-network/go-zenanet/eth/catalyst"
	...
	"github.com/zenanet-network/go-zenanet/ethdb/remotedb"
	"github.com/zenanet-network/go-zenanet/ethstats"
	"github.com/zenanet-network/go-zenanet/graphql"
	"github.com/zenanet-network/go-zenanet/internal/ethapi"
	"github.com/zenanet-network/go-zenanet/internal/flags"
	"github.com/zenanet-network/go-zenanet/log"
	"github.com/zenanet-network/go-zenanet/metrics"
	"github.com/zenanet-network/go-zenanet/metrics/exp"
	"github.com/zenanet-network/go-zenanet/metrics/influxdb"
...
var (
	...
	RPCGlobalTxFeeCapFlag = &cli.Float64Flag{
		Name:     "rpc.txfeecap",
		Usage:    "Sets a cap on transaction fee (in zena) that can be sent via the RPC APIs (0 = no cap)",
	...
	HttpHeaderFlag = &cli.StringSliceFlag{
		Name:     "header",
		Aliases:  []string{"H"},
		Usage:    "Pass custom headers to the RPC server when using --" + RemoteDBFlag.Name + " or the gze attach console. This flag can be given multiple times.",
		Category: flags.APICategory,
	}
...
// for Gze and returns half of the allowance to assign to the database.
...
// setZenabase retrieves the zenabase from the directly specified command line flags.
func setZenabase(ctx *cli.Context, cfg *ethconfig.Config) {
	if ctx.IsSet(MinerZenabaseFlag.Name) {
		log.Warn("Option --miner.zenabase is deprecated as the zenabase is set by the consensus client post-merge")
	}
...
func SetEthConfig(ctx *cli.Context, stack *node.Node, cfg *ethconfig.Config) {
	...
	case ctx.Bool(DeveloperFlag.Name):
		...
		// setZenabase has been called above, configuring the miner address from command line flags.
...
func MakeBeaconLightConfig(ctx *cli.Context) bparams.ClientConfig {
	...
	// Genesis root and time should always be specified togetherna with custom chain config
	if customConfig {
...
func SetupMetrics(ctx *cli.Context) {
	if metrics.Enabled {
		...
		if enableExport {
			...
			go influxdb.InfluxDBWithTags(metrics.DefaultRegistry, 10*time.Second, endpoint, database, username, password, "gze.", tagsMap)
		} else if enableExportV2 {
			tagsMap := SplitTagsFlag(ctx.String(MetricsInfluxDBTagsFlag.Name))

			log.Info("Enabling metrics export to InfluxDB (v2)")

			go influxdb.InfluxDBV2WithTags(metrics.DefaultRegistry, 10*time.Second, endpoint, token, bucket, organization, "gze.", tagsMap)
		}
...
func DialRPCWithHeaders(endpoint string, headers []string) (*rpc.Client, error) {
	...
	if strings.HasPrefix(endpoint, "rpc:") || strings.HasPrefix(endpoint, "ipc:") {
		// Backwards compatibility with gze < 1.5 which required
		// these prefixes.
		endpoint = endpoint[4:]
	}
