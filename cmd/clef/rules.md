...
* I want to auto-approve transactions with contract `CasinoDapp`, with up to `0.05 zena` in value to maximum `1 zena` per 24h period
...
implemented for `gze`. There are no known security vulnerabilities in it, nor have we had any security problems with it so far.
...
It's unclear whetherna any other DSL could be more secure; since there's always the possibility of erroneously implementing a rule.
