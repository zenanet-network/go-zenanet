...
import (
	...
	"github.com/zenanet-network/go-zenanet/internal/utesting"
...
func TestEthSuite(t *testing.T) {
	...
	gze, err := runGze("./testdata", jwtPath)
	if err != nil {
		t.Fatalf("could not run gze: %v", err)
	}
	defer gze.Close()

	suite, err := NewSuite(gze.Server().Self(), "./testdata", gze.HTTPAuthEndpoint(), common.Bytes2Hex(secret[:]))
	if err != nil {
...
// runGze creates and starts a gze node
func runGze(dir string, jwtPath string) (*node.Node, error) {
...
func setupGze(stack *node.Node, dir string) error {
