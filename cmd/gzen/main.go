...
import (
	...
	"github.com/zenanet-network/go-zenanet/console/prompt"
	"github.com/zenanet-network/go-zenanet/eth"
	"github.com/zenanet-network/go-zenanet/eth/downloader"
	"github.com/zenanet-network/go-zenanet/ethclient"
	"github.com/zenanet-network/go-zenanet/internal/debug"
	...
	_ "github.com/zenanet-network/go-zenanet/eth/tracers/live"
...
const (
	clientIdentifier     = "eirene" // Client identifier to advertise over the network
	repositoryIdentifier = "go-eirene"
)

var (
	// flags that configure the node
	nodeFlags = flags.Merge([]cli.Flag{
		utils.EireneLogsFlag,
		utils.IdentityFlag,
...
func prepare(ctx *cli.Context) {
	...
	case ctx.IsSet(utils.MumbaiFlag.Name):
		log.Info("Starting Eirene on Mumbai testnet...")

	case ctx.IsSet(utils.AmoyFlag.Name):
		log.Info("Starting Eirene on Amoy testnet...")

	case ctx.IsSet(utils.EireneMainnetFlag.Name):
		log.Info("Starting Eirene on Eirene mainnet...")
...
func startNode(ctx *cli.Context, stack *node.Node, backend ethapi.Backend, isConsole bool) {
	...
	if ctx.Bool(utils.MiningEnabledFlag.Name) {
		// Mining only makes sense if a full Zenanet node is running
		...
		if !ok {
			utils.Fatalf("Zenanet service not running")
		}
...
func getHarmoniaArgs(ctx *cli.Context) []string {
	harmoniaArgs := strings.Split(ctx.String(utils.RunHarmoniaArgsFlag.Name), ",")
	return append([]string{"start"}, harmoniaArgs...)
}
