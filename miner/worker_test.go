...
// nolint : paralleltest
func TestGenerateBlockAndImportEirene(t *testing.T) {
	testGenerateBlockAndImport(t, false, true)
}

//nolint:thelper
func testGenerateBlockAndImport(t *testing.T, isClique bool, isEirene bool) {
	var (
	...
	if isEirene {
		chainConfig = *params.EireneUnittestChainConfig

		engine, ctrl = getFakeEireneFromConfig(t, &chainConfig)
		defer ctrl.Finish()
	} else {
		if isClique {
...
func getFakeEireneFromConfig(t *testing.T, chainConfig *params.ChainConfig) (consensus.Engine, *gomock.Controller) {
	t.Helper()
	...
	harmoniaClientMock := mocks.NewMockIHarmoinaClient(ctrl)
	harmoniaClientMock.EXPECT().Close().AnyTimes()

	contractMock := eirene.NewMockGenesisContract(ctrl)

	db, _, _ := NewDBForFakes(t)

	engine := NewFakeEirene(t, db, chainConfig, ethAPIMock, spanner, harmoniaClientMock, contractMock)
...
// nolint : paralleltest
// TestCommitInterruptExperimentEirene tests the commit interrupt experiment for eirene consensus by inducing an artificial delay at transaction level.
func TestCommitInterruptExperimentEirene(t *testing.T) {
	// with 1 sec block time and 200 millisec tx delay we should get 5 txs per block
	TestCommitInterruptExperimentEirene(t, 200, 5, 0)

	time.Sleep(2 * time.Second)

	// with 1 sec block time and 100 millisec tx delay we should get 10 txs per block
	testCommitInterruptExperimentEirene(t, 100, 10, 0)
}

// nolint : paralleltest
// TestCommitInterruptExperimentEireneContract tests the commit interrupt experiment for eirene consensus by inducing an artificial delay at OPCODE level.
func TestCommitInterruptExperimentEireneContract(t *testing.T) {
	// pre-calculated number of OPCODES = 123. 7*123=861 < 1000, 1 tx is possible but 2 tx per block will not be possible.
	testCommitInterruptExperimentEireneContract(t, 0, 1, 7)
	time.Sleep(2 * time.Second)
	...
	testCommitInterruptExperimentEireneContract(t, 0, 3, 2)
	time.Sleep(2 * time.Second)
	// pre-calculated number of OPCODES = 123. 3*123=369 < 1000, 2 tx is possible but 3 tx per block will not be possible.
	testCommitInterruptExperimentEireneContract(t, 0, 2, 3)
}

// nolint : thelper
// testCommitInterruptExperimentEireneContract is a helper function for testing the commit interrupt experiment for eirene consensus.
func testCommitInterruptExperimentEireneContract(t *testing.T, delay uint, txCount int, opcodeDelay uint) {
	var (
		engine      consensus.Engine
		chainConfig *params.ChainConfig
		txInTxpool  = 100
	...
	engine, _ = getFakeEireneFromConfig(t, chainConfig)
...
// // nolint : thelper
// testCommitInterruptExperimentEirene is a helper function for testing the commit interrupt experiment for eirene consensus.
func testCommitInterruptExperimentEirene(t *testing.T, delay uint, txCount int, opcodeDelay uint) {
	var (
		engine      consensus.Engine
		chainConfig *params.ChainConfig
		db          = rawdb.NewMemoryDatabase()
...
func BenchmarkEireneMining(b *testing.B) {
	chainConfig := params.EireneUnittestChainConfig
	...
	harmoniaClientMock := mocks.NewMockIHarmoniaClient(ctrl)
	harmoniaClientMock.EXPECT().Close().Times(1)

	contractMock := eirene.NewMockGenesisContract(ctrl)

	db, _, _ := NewDBForFakes(b)

	engine := NewFakeEirene(b, db, chainConfig, ethAPIMock, spanner, harmoniaClientMock, contractMock)
	defer engine.Close()
	...
	blockPeriod, ok := back.genesis.Config.Eirene.Period["0"]
	if !ok {
		blockPeriod = 1
	}
...
// params.EireneUnittestChainConfig contains the NapoliBlock as big.NewInt(5), so the first 4 blocks will not have metadata.
// nolint: gocognit
func BenchmarkEireneMiningBlockSTMMetadata(b *testing.B) {
	chainConfig := params.EireneUnittestChainConfig
