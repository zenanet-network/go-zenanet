...
import (
	...
	"github.com/zenanet-network/go-zenanet/core/blockstm"
...
func (w *worker) mainLoop(ctx context.Context) {
	...
	for {
		...
		case req := <-w.newWorkCh:
			if w.chainConfig.ChainID.Cmp(params.EireneMainnetChainConfig.ChainID) == 0 || w.chainConfig.ChainID.Cmp(params.MumbaiChainConfig.ChainID) == 0 || w.chainConfig.ChainID.Cmp(params.AmoyChainConfig.ChainID) == 0 {
	...
	if EnableMVHashMap && w.IsRunning() {
		...
		if len(env.mvReadMapList) > 0 {
			...
			for i := 1; i <= len(env.mvReadMapList)-1; i++ {
				...
				_, ok2 := reads[blockstm.NewSubpathKey(common.HexToAddress(w.chainConfig.Eirene.CalculateBurntContract(env.header.Number.Uint64())), state.BalancePath)]
...
func (w *worker) prepareWork(genParams *generateParams) (*environment, error) {
	...
	if err := w.engine.Prepare(w.chain, header); err != nil {
		switch err.(type) {
		case *eirene.UnauthorizedSignerError:
		...
		go func(number uint64) {
			...
			for {
				...
				case <-time.After(150 * time.Millisecond):
					...
					dir, err := os.MkdirTemp("", fmt.Sprintf("eirene-traces-%s-", time.Now().UTC().Format("2006-01-02-150405Z")))
					if err != nil {
...
func (w *worker) commit(ctx context.Context, env *environment, interval func(), update bool, start time.Time) error {
	if w.IsRunning() {
		...
		// https://github.com/zenanet-network/go-zenanet/issues/24299
		env := env.copy()
