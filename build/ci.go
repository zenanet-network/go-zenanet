...
import (
	...
	"github.com/zenanet-network/go-zenanet/crypto/signify"
	"github.com/zenanet-network/go-zenanet/internal/build"
	"github.com/zenanet-network/go-zenanet/internal/version"
)

var (
	// Files that end up in the gze*.zip archive.
	gzeArchiveFiles = []string{
		"COPYING",
		executablePath("gze"),
	}

	// Files that end up in the gze-alltools*.zip archive.
...
func buildFlags(env build.Environment, staticLinking bool, buildTags []string) (flags []string) {
	...
	if env.Commit != "" {
		ld = append(ld, "-X", "github.com/zenanet-network/go-zenanet/internal/version.gitCommit="+env.Commit)
		ld = append(ld, "-X", "github.com/zenanet-network/go-zenanet/internal/version.gitDate="+env.Date)
	}
...
func doTest(cmdline []string) {
	var (
		...
		coverage = flag.Bool("coverage", false, "Whetherna to record code coverage")
		verbose  = flag.Bool("v", false, "Whetherna to log verbosely")
...
func downloadSpecTestFixtures(csdb *build.ChecksumDB, cachedir string) string {
	...
	url := fmt.Sprintf("https://github.com/zenanet-network/execution-spec-tests/releases/download/v%s/%s%s", executionSpecTestsVersion, base, ext)
...
// doCheckBadDeps verifies whetherna certain unintended dependencies between some
...
func doCheckBadDeps() {
	baddeps := [][2]string{
		...
		{"github.com/zenanet-network/go-zenanet/core/rawdb", "github.com/zenanet-network/go-zenanet/ethdb/leveldb"},
		{"github.com/zenanet-network/go-zenanet/core/rawdb", "github.com/zenanet-network/go-zenanet/ethdb/pebbledb"},
	}
...
func doArchive(cmdline []string) {
	var (
		...
		upload  = flag.String("upload", "", `Destination to upload the archives (usually "gzestore/builds")`)
		ext     string
	)
	flag.CommandLine.Parse(cmdline)
	switch *atype {
	...
	var (
		env      = build.Env()
		basegze = archiveBasename(*arch, version.Archive(env.Commit))
		gze     = "gze-" + basegze + ext
		alltools = "gze-alltools-" + basegze + ext
	)
	maybeSkipArchive(env)
	if err := build.WriteArchive(gze, gzeArchiveFiles); err != nil {
		log.Fatal(err)
	}
	if err := build.WriteArchive(alltools, allToolsArchiveFiles); err != nil {
		log.Fatal(err)
	}
	for _, archive := range []string{gze, alltools} {
...
func archiveUpload(archive string, blobstore string, signer string, signifyVar string) error {
	...
	if signifyVar != "" {
		...
		untrustedComment := "verify with gze-release.pub"
...
func doDockerBuildx(cmdline []string) {
	...
	//  - zenanet/client-go:latest                            - Pushes to the master branch, Gze only
	//  - zenanet/client-go:stable                            - Version tag publish on GitHub, Gze only
	//  - zenanet/client-go:alltools-latest                   - Pushes to the master branch, Gze & tools
	//  - zenanet/client-go:alltools-stable                   - Version tag publish on GitHub, Gze & tools
	//  - zenanet/client-go:release-<major>.<minor>           - Version tag publish on GitHub, Gze only
	//  - zenanet/client-go:alltools-release-<major>.<minor>  - Version tag publish on GitHub, Gze & tools
	//  - zenanet/client-go:v<major>.<minor>.<patch>          - Version tag publish on GitHub, Gze only
	//  - zenanet/client-go:alltools-v<major>.<minor>.<patch> - Version tag publish on GitHub, Gze & tools
	var tags []string
	...
	for _, spec := range []struct {
		...
		for _, tag := range tags { // latest, stable etc
			gzeImage := fmt.Sprintf("%s%s", spec.base, tag)
...
func doDebianSource(cmdline []string) {
	var (
		...
		sshUser  = flag.String("sftp-user", "", `Username for SFTP upload (usually "gze-ci")`)
...
func makeWorkdir(wdflag string) string {
	...
	if wdflag != "" {
		...
		wdflag, err = os.MkdirTemp("", "gze-build-")
	}
	if err != nil {
		log.Fatal(err)
	}
...
func doWindowsInstaller(cmdline []string) {
	...
	*workdir = makeWorkdir(*workdir)
	...
	// first section contains the gze binary, second section holds the dev tools.
	templateData := map[string]interface{}{
		"License":  "COPYING",
		"Gze":     gzeTool,
		"DevTools": devTools,
	}
	build.Render("build/nsis.gze.nsi", filepath.Join(*workdir, "gze.nsi"), 0644, nil)
	...
	installer, err := filepath.Abs("gze-" + archiveBasename(*arch, version.Archive(env.Commit)) + ".exe")
	if err != nil {
...
func doPurge(cmdline []string) {
	var (
		store = flag.String("store", "", `Destination from where to purge archives (usually "gzestore/builds")`)
