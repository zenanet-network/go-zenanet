...
import (
	...
	"github.com/zenanet-network/go-zenanet/accounts/scwallet"
	...
	"github.com/zenanet-network/go-zenanet/eth/gasestimator"
	"github.com/zenanet-network/go-zenanet/eth/tracers/logger"
...
func (api *BlockChainAPI) GetTransactionReceiptsByBlock(ctx context.Context, blockNrOrHash rpc.BlockNumberOrHash) ([]map[string]interface{}, error) {
	...
	for idx, receipt := range receipts {
		...
		if eireneReceipt != nil && idx == len(receipts)-1 {
...
func (api *BlockChainAPI) GetBlockByNumber(ctx context.Context, number rpc.BlockNumber, fullTx bool) (map[string]interface{}, error) {
	...
	if block != nil && err == nil {
		...
		// append marshalled eirene transaction
		if err == nil && response != nil {
			response = api.appendRPCMarshalEireneTransaction(ctx, block, response, fullTx)
		}
...
func (api *BlockChainAPI) GetBlockByHash(ctx context.Context, hash common.Hash, fullTx bool) (map[string]interface{}, error) {
	...
	if block != nil {
		...
		// append marshalled eirene transaction
		if err == nil && response != nil {
			return api.appendRPCMarshalEireneTransaction(ctx, block, response, fullTx), err
		}
...
func (api *BlockChainAPI) GetBlockReceipts(ctx context.Context, blockNrOrHash rpc.BlockNumberOrHash) ([]map[string]interface{}, error) {
	...
	stateSyncReceipt := rawdb.ReadEireneReceipt(api.b.ChainDb(), block.Hash(), block.NumberU64(), api.b.ChainConfig())
	if stateSyncReceipt != nil {
		tx, _, _, _ := rawdb.ReadEireneTransaction(api.b.ChainDb(), stateSyncReceipt.TxHash)
...
func DoCall(ctx context.Context, b Backend, args TransactionArgs, blockNrOrHash rpc.BlockNumberOrHash, state *state.StateDB, overrides *StateOverride, blockOverrides *BlockOverrides, timeout time.Duration, globalGasCap uint64) (*core.ExecutionResult, error) {
	...
	// EIRENE: This is used by eirene consensus to fetch data from genesis contracts for state-sync
	...
	if state == nil {
		...
		// when we're doing a call from eirene consensus to fetch data from genesis contracts. It's
...
func newRPCTransactionFromBlockIndex(b *types.Block, index uint64, config *params.ChainConfig, db ethdb.Database) *RPCTransaction {
	...
	var eireneReceipt *types.Receipt

	// Read eirene receipts if a state-sync transaction is requested
	if index == uint64(len(txs)) {
		eireneReceipt = rawdb.ReadEireneReceipt(db, b.Hash(), b.NumberU64(), config)
		if eireneReceipt != nil {
			if eireneReceipt.TxHash != (common.Hash{}) {
				eireneTx, _, _, _ := rawdb.ReadEireneTransactionWithBlockHash(db, eireneReceipt.TxHash, b.Hash())
				if eireneTx != nil {
					txs = append(txs, eireneTx)
				}
			}
	...
	// If the index is still out of the range after checking eirene state sync transaction, it means that the transaction index is invalid
	if index >= uint64(len(txs)) {
		return nil
	}
	...
	// If the transaction is a eirene transaction, we need to set the hash to the derived eirene tx hash. EireneTx is always the last index.
	if eireneReceipt != nil && index == uint64(len(txs)-1) {
		rpcTx.Hash = eireneReceipt.TxHash
		rpcTx.ChainID = nil
	}
...
func (api *TransactionAPI) GetTransactionByHash(ctx context.Context, hash common.Hash) (*RPCTransaction, error) {
	...
	// fetch eirene block tx if necessary
	if !found {
		if tx, blockHash, blockNumber, index, err = api.b.GetEireneBlockTransaction(ctx, hash); err != nil {
			return nil, err
		}
	...
	if tx != nil {
		...
		if eireneTx {
			...
			// In case of eirene block tx, we need simple derived tx hash (same as function argument) instead of RLP hash
			resultTx.Hash = hash
...
func (api *TransactionAPI) GetTransactionReceipt(ctx context.Context, hash common.Hash) (map[string]interface{}, error) {
	...
	if !found {
		tx, blockHash, blockNumber, index = rawdb.ReadEireneTransaction(api.b.ChainDb(), hash)
		eireneTx = true
	}
	...
	if eireneTx {
		// Fetch eirene block receipt
		receipt = rawdb.ReadEireneReceipt(api.b.ChainDb(), blockHash, blockNumber, api.b.ChainConfig())
	} else {
	...
	return marshalReceipt(receipt, blockHash, blockNumber, signer, tx, int(index), eireneTx), nil
}

// marshalReceipt marshals a transaction receipt into a JSON object.
func marshalReceipt(receipt *types.Receipt, blockHash common.Hash, blockNumber uint64, signer types.Signer, tx *types.Transaction, txIndex int, eireneTx bool) map[string]interface{} {
	from, _ := types.Sender(signer, tx)

	var txHash common.Hash

	if eireneTx {
		txHash = types.GetDerivedEireneTxHash(types.EireneReceiptKey(blockNumber, blockHash))
	} else {
		txHash = tx.Hash()
	}
...
//
// https://github.com/zenanet-network/wiki/wiki/JSON-RPC#eth_sign
