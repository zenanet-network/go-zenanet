...
func (s *BlockChainAPI) GetEireneBlockReceipt(ctx context.Context, hash common.Hash) (*types.Receipt, error) {
	return s.b.GetEireneBlockReceipt(ctx, hash)
}
...
//
// Eirene transaction utils
//

func (s *BlockChainAPI) appendRPCMarshalEireneTransaction(ctx context.Context, block *types.Block, fields map[string]interface{}, fullTx bool) map[string]interface{} {
	if block != nil {
		txHash := types.GetDerivedEireneTxHash(types.EireneReceiptKey(block.Number().Uint64(), block.Hash()))

		eireneTx, blockHash, blockNumber, txIndex, _ := s.b.GetEireneBlockTransactionWithBlockHash(ctx, txHash, block.Hash())
		if eireneTx != nil {
			formattedTxs := fields["transactions"].([]interface{})

			if fullTx {
				marshalledTx := newRPCTransaction(eireneTx, blockHash, blockNumber, block.Time(), txIndex, block.BaseFee(), s.b.ChainConfig())
...
// EireneAPI provides an API to access Eirene related information.
type EireneAPI struct {
	b Backend
}

// NewEireneAPI creates a new Eirene protocol API.
func NewEireneAPI(b Backend) *EireneAPI {
	return &EireneAPI{b}
}
...
func (api *EireneAPI) SendRawTransactionConditional(ctx context.Context, input hexutil.Bytes, options types.OptionsPIP15) (common.Hash, error) {
...
func (api *EireneAPI) GetVoteOnHash(ctx context.Context, starBlockNr uint64, endBlockNr uint64, hash string, milestoneId string) (bool, error) {
