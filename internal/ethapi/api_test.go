...
import (
	...
	"github.com/zenanet-network/go-zenanet/internal/blocktest"
...
func newTestBackend(t *testing.T, n int, gspec *core.Genesis, engine consensus.Engine, generator func(i int, b *core.BlockGen)) *testBackend {
	...
	gspec.Alloc[acc.Address] = types.Account{Balance: big.NewInt(params.Zena)}
...
func TestEstimateGas(t *testing.T) {
	...
	var (
		...
		genesis  = &core.Genesis{
			...
			Alloc: types.GenesisAlloc{
				accounts[0].addr: {Balance: big.NewInt(params.Zena)},
				accounts[1].addr: {Balance: big.NewInt(params.Zena)},
			},
		}
		genBlocks      = 10
	...
	var testSuite = []struct {
		...
		},
		{
			...
			call:        TransactionArgs{},
			overrides: StateOverride{
				randomAccounts[0].addr: OverrideAccount{Balance: newRPCBalance(new(big.Int).Mul(big.NewInt(1), big.NewInt(params.Zena)))},
			},
			expectErr: nil,
			want:      53000,
		},
	...
	var (
		...
		genesis  = &core.Genesis{
			...
			Alloc: types.GenesisAlloc{
				accounts[0].addr: {Balance: big.NewInt(params.Zena)},
				accounts[1].addr: {Balance: big.NewInt(params.Zena)},
				accounts[2].addr: {Balance: big.NewInt(params.Zena)},
				dad: {
					Balance: big.NewInt(params.Zena),
					Nonce:   1,
	...
	var (
		...
		genesis      = &core.Genesis{
			...
			Alloc: types.GenesisAlloc{
				...
				//     }
				// }
				cac: {Balance: big.NewInt(params.Zena), Code: common.Hex2Bytes("610dad80ff")},
				bab: {
...
func TestRPCGetBlockOrHeader(t *testing.T) {
	...
	var (
		...
		genesis    = &core.Genesis{
			Config: params.TestChainConfig,
			Alloc: types.GenesisAlloc{
				acc1Addr: {Balance: big.NewInt(params.Zena)},
				acc2Addr: {Balance: big.NewInt(params.Zena)},
			},
		}
		genBlocks = 10
		signer    = types.HomesteadSigner{}
	...
	var (
		...
		genesis    = &core.Genesis{
			...
			Alloc: types.GenesisAlloc{
				...
				//     }
				// }
				contract: {Balance: big.NewInt(params.Zena), Code: common.FromHex("")},
			},
		}
