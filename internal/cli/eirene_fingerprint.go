...
func (c *FingerprintCommand) Help() string {
	return `Usage: eirene fingerprint
...
func formatFingerprint(eireneFingerprint *EireneFingerprint) string {
	base := formatKV([]string{
		fmt.Sprintf("Eirene Version : %s", params.VersionWithMeta),
		fmt.Sprintf("CPU : %d cores", eireneFingerprint.CoresCount),
		fmt.Sprintf("OS : %s %s ", eireneFingerprint.OsName, eireneFingerprint.OsVer),
		fmt.Sprintf("RAM :: total : %v GB, free : %v GB, used : %v GB", eireneFingerprint.MemoryDetails.TotalMem, eireneFingerprint.MemoryDetails.FreeMem, eireneFingerprint.MemoryDetails.UsedMem),
		fmt.Sprintf("STORAGE :: total : %v GB, free : %v GB, used : %v GB", eireneFingerprint.DiskDetails.TotalDisk, eireneFingerprint.DiskDetails.FreeDisk, eireneFingerprint.DiskDetails.UsedDisk),
	})
...
func (c *FingerprintCommand) Run(args []string) int {
	...
	eireneFingerprint := &EireneFingerprint{
	...
	c.UI.Output(formatFingerprint(eireneFingerprint))

	if eireneFingerprint.OsName == "linux" {
		err = c.benchmark()
		if err != nil {
			c.UI.Error(err.Error())
			return 1
