...
import (
	...
	"github.com/zenanet-network/go-zenanet/internal/cli/flagset"
	"github.com/zenanet-network/go-zenanet/internal/cli/server"
...
func (c *SnapshotCommand) Help() string {
	return `Usage: eirene snapshot <subcommand>

  This command groups snapshot related actions.

  Prune the state trie:

    $ eirene snapshot prune-state

  Prune the ancient data:

    $ eirene snapshot prune-block

  Inspect ancient DB pruning related fields:

    $ eirene snapshot inspect-ancient-db`
}
...
func (c *PruneStateCommand) MarkDown() string {
	items := []string{
		`# Prune state`,
		`The ` + "```" + "eirene snapshot prune-state" + "```" + ` command will prune historical state data
...
func (c *PruneStateCommand) Help() string {
	return `Usage: eirene snapshot prune-state <datadir>
...
func (c *PruneBlockCommand) MarkDown() string {
	items := []string{
		...
		"The ```eirene snapshot prune-block``` command will prune historical blockchain data stored in the ancientdb. The amount of blocks expected for remaining after prune can be specified via `block-amount-reserved` in this command, will prune and only remain the specified amount of old block data in ancientdb.",
		`
...
func (c *PruneBlockCommand) Help() string {
	return `Usage: eirene snapshot prune-block <datadir>
...
func (c *InspectAncientDbCommand) MarkDown() string {
	items := []string{
		...
		"The ```eirene snapshot inspect-ancient-db``` command will inspect few fields in the ancient datastore using the given datadir location.",
		`
...
func (c *InspectAncientDbCommand) Help() string {
	return `Usage: eirene snapshot inspect-ancient-db <datadir>
