...
// AttachCommand is the command to Connect to remote Eirene IPC console
...
func (c *AttachCommand) MarkDown() string {
	items := []string{
		"# Attach",
		"Connect to remote Eirene IPC console.",
		c.Flags().MarkDown(),
	}
...
func (c *AttachCommand) Help() string {
	return `Usage: Eirene attach <IPC FILE>
...
func (c *AttachCommand) Synopsis() string {
	return "Connect to Eirene via IPC"
}
...
// remoteConsole will connect to a remote eirene instance, attaching a JavaScript
// console to it.
// nolint: unparam
func (c *AttachCommand) remoteConsole() error {
	...
	if c.Endpoint == "" {
		...
		if path != "" {
			...
			path = filepath.Join(homeDir, "/.eirene/data")
		}

		c.Endpoint = fmt.Sprintf("%s/eirene.ipc", path)
	}

	client, err := dialRPC(c.Endpoint)

	if err != nil {
		utils.Fatalf("Unable to attach to remote eirene: %v", err)
	}
...
func dialRPC(endpoint string) (*rpc.Client, error) {
	if endpoint == "" {
		endpoint = node.DefaultIPCEndpoint("eirene")
