...
func (c *DebugBlockCommand) MarkDown() string {
	items := []string{
		"# Debug trace",
		"The ```eirene debug block <number>``` command will create an archive containing traces of a eirene block.",
		c.Flags().MarkDown(),
	}
...
func (c *DebugBlockCommand) Help() string {
	return `Usage: eirene debug block <number>

  This command is used get traces of a eirene block`
}
...
func (c *DebugBlockCommand) Synopsis() string {
	return "Get trace of a eirene block"
}
...
func (c *DebugBlockCommand) Run(args []string) int {
	...
	stream, err := eireneClt.DebugBlock(context.Background(), debugRequest)
	if err != nil {
		c.UI.Error(err.Error())
		return 1
	}
