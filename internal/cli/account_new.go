...
func (a *AccountNewCommand) MarkDown() string {
	items := []string{
		"# Account new",
		"The `account new` command creates a new local account file on the Eirene data directory. Eirene should not be running to execute this command.",
		a.Flags().MarkDown(),
	}
...
func (a *AccountNewCommand) Help() string {
	return `Usage: eirene account new
