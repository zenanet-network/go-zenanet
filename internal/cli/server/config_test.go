...
func TestConfigDefault(t *testing.T) {
	...
	assert.NoError(t, err)
	assertEireneDefaultGasPrice(t, ethConfig)
}

// assertEireneDefaultGasPrice asserts the eirene default gas price is set correctly.
func assertEireneDefaultGasPrice(t *testing.T, ethConfig *ethconfig.Config) {
	assert.NotNil(t, ethConfig)
	assert.Equal(t, ethConfig.Miner.GasPrice, big.NewInt(params.EireneDefaultMinerGasPrice))
}
