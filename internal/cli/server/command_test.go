...
func TestFlagsWithoutConfig(t *testing.T) {
	...
	require.Equal(t, c.config.EireneLogs, false)
	...
	require.Equal(t, c.config.JsonRPC.Http.API, []string{"eth", "eirene"})
...
func TestFlagsWithConfigAndFlags(t *testing.T) {
	...
	require.Equal(t, c.config.EireneLogs, true)
	...
	require.Equal(t, c.config.JsonRPC.Ws.API, []string{"eth", "eirene", "web3"})
