...
import (
	...
	"github.com/zenanet-network/go-zenanet/internal/cli/server/chains"
	...
	"github.com/zenanet-network/go-zenanet/p2p/nat"
	"github.com/zenanet-network/go-zenanet/p2p/netutil"
...
type Config struct {
	...
	// EireneLogs enables eirene log retrieval
	EireneLogs bool `hcl:"eirene.logs,optional" toml:"eirene.logs,optional"`
	...
	// Harmonia has the harmonia connection related settings
	Harmonia *HarmoniaConfig `hcl:"harmonia,block" toml:"harmonia,block"`
...
type HarmoniaConfig struct {
	// URL is the url of the harmonia server
	URL string `hcl:"url,optional" toml:"url,optional"`

	// Without is used to disable remote harmonia during testing
	Without bool `hcl:"eirene.without,optional" toml:"eirene.without,optional"`

	// GRPCAddress is the address of the harmonia grpc server
	GRPCAddress string `hcl:"grpc-address,optional" toml:"grpc-address,optional"`

	// RunHarmonia is used to run harmonia as a child process
	RunHarmonia bool `hcl:"eirene.runharmonia,optional" toml:"eirene.runharmonia,optional"`

	// RunHarmonia args are the arguments to run harmonia with
	RunHarmoniaArgs string `hcl:"eirene.runharmoniaargs,optional" toml:"eirene.runharmoniaargs,optional"`

	// UseHarmoniaApp is used to fetch data from harmonia app when running harmonia as a child process
	UseHarmoniaApp bool `hcl:"eirene.useharmoniaapp,optional" toml:"eirene.useharmoniaapp,optional"`
}
...
type AccountsConfig struct {
	...
	// DisableEireneWallet disables the personal wallet endpoints
	DisableEireneWallet bool `hcl:"disable-eirene-wallet,optional" toml:"disable-eirene-wallet,optional"`
}
...
func DefaultConfig() *Config {
	return &Config{
		...
		TxPool: &TxPoolConfig{
			...
			PriceLimit:   params.EireneDefaultTxPoolPriceLimit, // eirene's default
			PriceBump:    10,
			AccountSlots: 16,
			GlobalSlots:  131072,
			AccountQueue: 64,
		...
		Sealer: &SealerConfig{
			...
			GasPrice:            big.NewInt(params.EireneDefaultMinerGasPrice), // eirene's default
			ExtraData:           "",
		...
		Gpo: &GpoConfig{
			...
			IgnorePrice:      gasprice.DefaultIgnorePrice, // eirene's default
		},
		JsonRPC: &JsonRPCConfig{
			...
			Http: &APIConfig{
				...
				Prefix:                      "",
				Host:                        "localhost",
				API:                         []string{"eth", "net", "web3", "txpool", "eirene"},
...
func (c *Config) buildEth(stack *node.Node, accountManager *accounts.Manager) (*ethconfig.Config, error) {
	...
	n.HarmoniaURL = c.Harmonia.URL
	n.WithoutHarmonia = c.Harmonia.Without
	n.HarmoniagRPCAddress = c.Harmonia.GRPCAddress
	n.RunHarmonia = c.Harmonia.RunHarmonia
	n.RunHarmoniaArgs = c.Harmonia.RunHarmoniaArgs
	n.UseHarmoniaApp = c.Harmonia.UseHarmoniaApp

	// Developer Fake Author for producing blocks without authorisation on eirene consensus
	...
	case "snap":
		...
		log.Warn("Eirene doesn't support Snap Sync yet, switching to Full Sync mode")
	default:
...
func (c *Config) buildNode() (*node.Config, error) {
	...
	if c.Developer.Enabled {
		...
		cfg.HTTPModules = []string{"admin", "debug", "eth", "miner", "net", "personal", "txpool", "web3", "eirene"}
	}
...
func DefaultDataDir() string {
	...
	switch runtime.GOOS {
	case "darwin":
		return filepath.Join(home, "Library", "Eirene")
	case "windows":
		...
		return filepath.Join(appdata, "Eirene")
	default:
		return filepath.Join(home, ".eirene")
	}
}
