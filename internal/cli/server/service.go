...
var ErrUnavailable = errors.New("eirene service is currently unavailable, try again later")
var ErrUnavailable2 = errors.New("eirene service unavailable even after waiting for 10 seconds, make sure eirene is running")

func sendStreamDebugFile(stream proto.Eirene_DebugPprofServer, headers map[string]string, data []byte) error {
...
func (s *Server) Status(ctx context.Context, in *proto.StatusRequest) (*proto.StatusResponse, error) {
	...
	resp := &proto.StatusResponse{
		...
		Forks: gatherForks(s.config.chain.Genesis.Config, s.config.chain.Genesis.Config.Eirene),
	}
...
func (s *Server) DebugBlock(req *proto.DebugBlockRequest, stream proto.Eirene_DebugBlockServer) error {
	...
	res, err := s.tracerAPI.TraceEireneBlock(traceReq)
	if err != nil {
		return err
	}
