...
func (c *Command) MarkDown() string {
	items := []string{
		"# Server",
		"The ```eirene server``` command runs the Eirene client.",
...
func (c *Command) Help() string {
	return `Usage: eirene [options]

	Run the Eirene server.
  ` + c.Flags(nil).Help()
}

// Synopsis implements the cli.Command interface
func (c *Command) Synopsis() string {
	return "Run the Eirene server"
}
...
func (c *Command) getHarmoniaArgs() []string {
	harmoniaArgs := strings.Split(c.config.Harmonia.RunHarmoniaArgs, ",")
