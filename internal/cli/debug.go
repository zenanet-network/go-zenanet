...
func (c *DebugCommand) MarkDown() string {
	examples := []string{
		...
		CodeBlock([]string{
			"$ eirene debug",
			"Starting debugger...\n",
			"Created debug archive: eirene-debug-2021-10-26-073819Z.tar.gz",
		}),
		"Send the output to a specific directory:",
		CodeBlock([]string{
			"$ eirene debug --output data",
			"Starting debugger...\n",
			"Created debug directory: data/eirene-debug-2021-10-26-075437Z",
		}),
	}

	items := []string{
		"# Debug",
		"The ```eirene debug``` command takes a debug dump of the running client.",
		"- [```eirene debug pprof```](./debug_pprof.md): Dumps eirene pprof traces.",
		"- [```eirene debug block <number>```](./debug_block.md): Dumps eirene block traces.",
	}
