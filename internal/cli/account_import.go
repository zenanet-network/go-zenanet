...
func (a *AccountImportCommand) MarkDown() string {
	items := []string{
		"# Account import",
		"The ```account import``` command imports an account in Json format to the Eirene data directory.",
		a.Flags().MarkDown(),
	}
...
func (a *AccountImportCommand) Help() string {
	return `Usage: eirene account import

  Import a private key into a new account.

  Import an account:

    $ eirene account import key.json
