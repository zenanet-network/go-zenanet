...
import (
	...
	"github.com/zenanet-network/go-zenanet/version"
)

const ourPath = "github.com/zenanet-network/go-zenanet" // Path to our module
...
// Archive holds the textual version string used for Gze archives. e.g.
...
// version and whetherna it was replaced by another module.
...
func versionInfo(info *debug.BuildInfo) string {
	// If the main package is from our repo, prefix version with "gze".
	if strings.HasPrefix(info.Path, ourPath) {
		return fmt.Sprintf("gze %s", info.Main.Version)
	}
	...
	if mod == nil {
		...
		// version.
		return version + fmt.Sprintf("gze %s", WithMeta)
	}
