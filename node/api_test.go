...
// checking whetherna the HTTP server is started correctly.
func TestStartRPC(t *testing.T) {
	type test struct {
		...
		wantReachable bool // whetherna the HTTP server should be reachable at all
		wantHandlers  bool // whetherna RegisterHandler handlers should be accessible
		wantRPC       bool // whetherna JSON-RPC/HTTP should be accessible
		wantWS        bool // whetherna JSON-RPC/WS should be accessible
	}
...
// checkBodyOK checks whetherna the given HTTP URL responds with 200 OK and body "OK".
...
// checkRPC checks whetherna JSON-RPC works against the given URL.
