...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of gze is "gze". If no
...
// ExtRPCEnabled returns the indicator whetherna node enables the external
...
func (c *Config) NodeName() string {
	name := c.name()
	// Backwards compatibility: previous versions used title-cased "Gze", keep that.
	if name == "gze" || name == "gze-testnet" {
		name = "Gze"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "gze" instances.
var isOldGzeResource = map[string]bool{
	"chaindata":          true,
	"nodes":              true,
	"nodekey":            true,
...
func (c *Config) ResolvePath(path string) string {
	...
	// by gze 1.4 are used if they exist.
	if warn, isOld := isOldGzeResource[path]; isOld {
		oldpath := ""
		if c.name() == "gze" {
			oldpath = filepath.Join(c.DataDir, path)
		}
		if oldpath != "" && common.FileExist(oldpath) {
			if warn && !c.oldGzeResourceWarning {
				c.oldGzeResourceWarning = true
				log.Warn("Using deprecated resource file, please move this file to the 'gze' subdirectory of datadir.", "file", oldpath)
			}
			return oldpath
		}
	}
